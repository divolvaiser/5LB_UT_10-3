...
Функция ОбработатьВведенныйШтрихкод2(Штрихкод, Клиент, НайденнаяНоменк) Экспорт

	НайденнаяНоменк = Неопределено;
	
	Если Не ЗначениеЗаполнено(Штрихкод) Тогда
		Возврат НайденнаяНоменк;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ТипШК) Тогда
		//Если РаботаСТорговымОборудованием.ТипШтрихкодаEAN13ТО(ШК) Тогда
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				РезультатЗапроса = РезультатЗапроса.Выбрать();
				РезультатЗапроса.Следующий();
				НайденнаяНоменк = РезультатЗапроса.Владелец
			Иначе
				Клиент.СШКОшибка(мОшибкаНеизвестныйШтрихкод, ШК, ТипШК, СШК);
				//Сообщить ("Штрих-код не найден в базе",СтатусСообщения.Важное);
			КонецЕсли;
...
КонецФункции // ОбработатьВведенныйШтрихкод2()

//11.10.2017, Юра
Функция ОбработатьВведенныйШтрихКодВыдатьСтруктуру(ШтрихКод) Экспорт
	Если Не ЗначениеЗаполнено(Штрихкод) Тогда
	...
	Если РезультатЗапроса.Пустой() Тогда
		Сообщить ("Для номенклатуры штрих-код "+ШтрихКод+" в базе не найден.",СтатусСообщения.Важное);
		Возврат Неопределено;
	Иначе
		Выборка = РезультатЗапроса.Выбрать();
		Выборка.Следующий();
		Возврат Новый Структура("Номенклатура,ЕдиницаИзмерения,Характеристика,Серия,Качество",Выборка.Номенклатура,Выборка.ЕдиницаИзмерения,Выборка.Характеристика,Выборка.Серия,Выборка.Качество);
	КонецЕсли;

...
Функция СоединениеСАльтернативнымСерверомОбновлений() Экспорт
	...
	// Создание HTTP-соединения с сервером обновлений
	Соединение = Новый HTTPСоединение(ОбновлениеКонфигурации.АльтернативныйСерверОбновлений(),
	                                  ,
	                                  КодПользователяСервераОбновлений,
	                                  ПарольСервераОбновлений,
	                                  ?(ПроксиСерверТребуетАутентификации, ПроксиСервер, Неопределено));
...
Функция СоединениеССерверомДляПроверкиНаличияОбновления()
	...
	// Создание HTTP-соединения с сервером обновлений
	Соединение = Новый HTTPСоединение(ОбновлениеКонфигурации.АдресСервераДляПроверкиНаличияОбновления(),
	                                  ,
	                                  ,
	                                  ,
	                                  ?(ПроксиСерверТребуетАутентификации, ПроксиСервер, Неопределено));
...
Функция ПолучитьФайлОписанияОбновления() Экспорт
	...
	ТекстОшибки = ПолучитьФайлССервера(ОбновлениеКонфигурации.АдресРесурсовДляПроверкиНаличияОбновления() + ИмяФайлаОписанияОбновления, 
		ИмяФайла, Ложь);
...
Функция ПолучитьФайлПорядкаОбновления() Экспорт
	...
	ТекстОшибки = ПолучитьФайлССервера(ОбновлениеКонфигурации.АдресРесурсовДляПроверкиНаличияОбновления() + ИмяФайлаПорядкаОбновления, 
		ИмяФайла, Ложь);
	Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
		Попытка
			УдалитьФайлы(ИмяФайла);
...
Функция ПолучитьФайлСпискаШаблонов(Знач ВыдаватьСообщения = Истина) Экспорт
	...
	ТекстОшибки = ПолучитьФайлССервера(ОбновлениеКонфигурации.АдресРесурсовДляПроверкиНаличияОбновления() + ИмяZipФайлаСпискаШаблонов,
...
Функция ПолучитьПараметрыОбновления(Знач ВыдаватьСообщения = Истина) Экспорт
	...
	ТекстОшибки = ПолучитьФайлССервера(ОбновлениеКонфигурации.АдресРесурсовДляПроверкиНаличияОбновления() + ОбновлениеКонфигурации.ИмяФайлаПроверкиНаличияОбновления(),
...
Функция СоздатьСписокФайловДляПолучения(ИсточникИнтернет) Экспорт 
	...
	КаталогОбновленияНаСервере = ?(ИсточникИнтернет, ОбновлениеКонфигурации.АдресКаталогаШаблоновНаСервереОбновлений(), 
	...
	Для Каждого Обновление Из ДоступныеОбновления Цикл
		...
		НовыйФайл.АльтернативныйАдрес = ?(ИсточникИнтернет, 
			ОбновлениеКонфигурации.АльтернативныйКаталогШаблоновНаСервереОбновлений() + Обновление.ПутьКФайлуОбновления, "");
	...
	Если ИсточникИнтернет Тогда
		...
		НовыйФайл = СписокФайлов.Добавить();
		НовыйФайл.Адрес         = ОбновлениеКонфигурации.АдресКаталогаШаблоновНаСервереОбновлений() + Лев(Обновление.ПутьКФайлуОбновления, Поз-1) + ФайлАрхиваРекламы;
		НовыйФайл.АльтернативныйАдрес = ОбновлениеКонфигурации.АльтернативныйКаталогШаблоновНаСервереОбновлений() + 

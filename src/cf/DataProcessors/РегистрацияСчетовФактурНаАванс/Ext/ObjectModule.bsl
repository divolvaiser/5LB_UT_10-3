...
Процедура ЗаполнитьСтрокиДокумента(ТаблицаРезультатов) Экспорт
	...
	|		НДСРасчетыСПокупателямиОстатки.ВалютаРасчетов КАК ВалютаРасчетов,
	|		-1 * ВЫБОР
	...
	|		НДСРасчетыСПокупателямиОстатки.ДоговорКонтрагента.РасчетыВУсловныхЕдиницах КАК РасчетыВУсловныхЕдиницах,
	|		ВЫБОР
	...
	|				ИЛИ НДСРасчетыСПокупателямиОстатки.ВалютнаяСуммаНачальныйОстаток < 0)) КАК Расчеты
	|ГДЕ
	|	ВЫБОР
...
Процедура ОпределитьРанееВыписанныеСчетаФактуры(ТаблицаАвансов)
	...
	Если ТаблицаАвансов.Количество() > 0 Тогда
		...
		Запрос.Текст = 
			...
			|	И СчетФактураВыданный.Ссылка.НаАванс = ИСТИНА

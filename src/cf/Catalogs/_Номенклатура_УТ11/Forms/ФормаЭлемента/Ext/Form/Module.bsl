...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	КарточкаНоменклатуры = Справочники.Номенклатура.ПолучитьТабличныйДокументКарточкиНоменклатуры(Объект);
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Если Объект.НаборУпаковок = Справочники.НаборыУпаковок.ИндивидуальныйДляНоменклатуры Тогда
		ИспользованиеУпаковок = НСтр("ru = 'Индивидуальный набор'");
	Иначе
		ИспользованиеУпаковок = НСтр("ru = 'Общий набор'");
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ЗапретРедактированияРеквизитовОбъектов.ЗаблокироватьРеквизиты(ЭтаФорма);
	НастроитьДоступностьБлокируемыхРеквизитов();
	КарточкаНоменклатуры = Справочники.Номенклатура.ПолучитьТабличныйДокументКарточкиНоменклатуры(ТекущийОбъект);
...
Процедура ИспользоватьУпаковкиПриИзменении(Элемент)
	...
	Иначе
		...
		Если Объект.НаборУпаковок = ПредопределенноеЗначение("Справочник.НаборыУпаковок.ИндивидуальныйДляНоменклатуры") Тогда
			ИспользованиеУпаковок = "Индивидуальный набор";
		КонецЕсли;
...
Процедура ИспользованиеУпаковокОбщийНаборПриИзменении(Элемент)
	ИспользованиеУпаковок = "Общий набор";
	Объект.НаборУпаковок = Неопределено;
	НастроитьФорму();
КонецПроцедуры
...
Процедура НастроитьФорму()
	...
	Элементы.ЕдиницаИзмерения.Доступность = (ИспользованиеУпаковок = "Индивидуальный набор"
...
Процедура СкрытьРаскрытьВсеГруппы(Свернуть = Неопределено)
	...
	ИзменитьСвернутостьГруппы("Алкоголь",Свернуть);
	ИзменитьСвернутостьГруппы("СведенияОПроизовдителе",Свернуть);
	ИзменитьСвернутостьГруппы("Классификаторы",Свернуть);
...
Процедура ПроверитьЗаполнениеОбязательныхРеквизитов(Отказ)
	...
	Если Не ЗначениеЗаполнено(Объект.ШаблонЭтикетки) И (ИспользоватьИндивидуальныйШаблонЭтикетки = 1) Тогда
		ТекстСообщения = СтрЗаменить(ТекстОшибки, "%ИмяРеквизита%", НСтр("ru = 'Шаблон этикетки'"));
	...
	Если Не ЗначениеЗаполнено(Объект.ШаблонЦенника) И (ИспользоватьИндивидуальныйШаблонЦенника = 1) Тогда
		ТекстСообщения = СтрЗаменить(ТекстОшибки, "%ИмяРеквизита%", НСтр("ru = 'Шаблон ценника'"));

...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.Числитель / Объект.Знаменатель = 1
		...
		Если Не ЕстьЕдиничнаяУпаковкаСЕдиницейИзмерения(Объект.Ссылка, Объект.Владелец, Объект.ЕдиницаИзмерения) 
			...
			Иначе
				ТекстСообщения = НСтр("ru = 'Для вида номенклатуры ""%Владелец%"" уже создана упаковка с коэффициентом 1 и единицей измерения ""%ЕдиницаИзмерения%"".?'");
			КонецЕсли;
...
Процедура НастроитьФорму()
	...
	Иначе
		ПредставлениеЕдиницыИзмерения =  НСтр("ru = '<ед. изм. по классификтору>'");
	КонецЕсли;

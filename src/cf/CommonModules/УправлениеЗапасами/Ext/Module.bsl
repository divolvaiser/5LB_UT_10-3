...
//Транзит товара до магазинов
Процедура ТранзитПеремещениеПриЗаписи(Источник, Отказ) Экспорт
	#Если Клиент Тогда
		Если (Источник.ВидОперации = Перечисления.ВидыОперацийПеремещениеТоваров.Транзит) И (Источник.Проведен=Ложь) Тогда
			Движение = РегистрыНакопления.ТоварыНаПолке.СоздатьНаборЗаписей();
			Движение.Отбор.Регистратор.Установить(Источник.Ссылка);
			Для каждого запись Из Источник.Товары Цикл
				Если запись.КоличествоСобрано>0 Тогда
					ЗаписьРегистра = Движение.ДобавитьПриход();
					ЗаписьРегистра.Активность = Истина;
					ЗаписьРегистра.Период = Источник.Дата;
					ЗаписьРегистра.Склад = Источник.СкладОтправитель;
					ЗаполнитьЗначенияСвойств(ЗаписьРегистра,Запись);
					ЗаписьРегистра.ДокументРезерва = Источник.Ссылка;
				КонецЕсли;
			КонецЦикла;
			Движение.Записать();
		ИначеЕсли  Источник.Проведен=Ложь Тогда
			Движение = РегистрыНакопления.ТоварыНаПолке.СоздатьНаборЗаписей();
			Движение.Отбор.Регистратор.Установить(Источник.Ссылка);
            Движение.Записать();
		КонецЕсли;
	...
	//			Попытка
	//				ТекДок.Записать(РежимЗаписиДокумента.Проведение,РежимПроведенияДокумента.Оперативный);
	//				Источник.ВнутреннийЗаказ = ТекДок.Ссылка;

...
Процедура РассчитатьСкидкиПриПродаже(ДокументОбъект, ТабличнаяЧастьНоменклатуры,
	...
	Для Каждого СтрокаТЧ Из ТаблицаТоваров Цикл
		//{+НАЧАЛО+Добавления} Автор: Тасмаджиев 22/05/2018//==============================================================================================
		//================================================!{Отказ от скидки по определенной номенклатуре}!=====================================================
		//Если в РегистрыСведений._5LB_Исключение_Скидки_Номенклатуры находит номенклатуру из Таб части то на нее не действует скидка.
		//02112017 теперь и серитфикаты, и карты
		СтруктураП = Новый Структура;
		СтруктураП.Вставить("Номенклатура",СтрокаТЧ.Номенклатура);
		Выборка = РегистрыСведений._5LB_Исключение_Скидки_Номенклатуры.Выбрать(СтруктураП);
		Если Выборка.Следующий() Тогда
			Продолжить;
		КонецЕсли;
		//{-КОНЕЦ-Добавления} Автор: Тасмаджиев 22/05/2018//==============================================================================================

		Если СтрокаТЧ.Номенклатура.Родитель = Справочники.Номенклатура.НайтиПоКоду("6217") Тогда
			продолжить;
		КонецЕсли;
...
Процедура РассчитатьСуммуТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, СпособРасчета = Неопределено) Экспорт
    ...
    // если мы получили сумму скидки с SP, тогда неверно ее пересчитывать по процентам
    Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СуммаСкидкиSailPlay", МетаданныеДокумента, ИмяТабличнойЧасти)
        И СтрокаТабличнойЧасти.СуммаСкидкиSailPlay > 0 Тогда
        СуммаСкидки = СтрокаТабличнойЧасти.СуммаСкидкиSailPlay;
    КонецЕсли; 
    //  end ins SailPlay 04.07.2016 
	
	// ins SailPlay 23.04.2016 
    СуммаБонусами = 0;
    Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СуммаСкидкиОплатыБонусомSailPlay", МетаданныеДокумента, ИмяТабличнойЧасти) 
		И СтрокаТабличнойЧасти.СуммаСкидкиОплатыБонусомSailPlay > 0 Тогда
        СуммаБонусами = СтрокаТабличнойЧасти.СуммаСкидкиОплатыБонусомSailPlay;
        СуммаСкидки = СуммаСкидки + СуммаБонусами;
    КонецЕсли;    
    //  end ins SailPlay 23.04.2016 
...
// Рассчитывает сумму(Передаваемый реквизит) в строке табличной части документа - (03.03.2016 - Кузнецов С.А)
//
// Параметры:
//  СтрокаТабличнойЧасти - строка табличной части документа,
//  ИмяРеквизитаСуммы    - имя реквизита табличной части, которую необходимо рассчитать (Сумма)    
//  ИмяРеквизитаЦены     - имя реквизита табличной части, исходя из которой необходимо рассчитать сумму (Цена)
//  ДокументОбъект       - объект редактируемого документа,
//  СпособРасчета        - "Со всеми скидками", сумма минус скидки;
//                         "Без учета ручной скидки", сумма минус автоматические скидки;
//                         "Без учета скидок", сумма (Цена * Количество);
//
Процедура РассчитатьСуммуТабЧастиПоИмениРеквизита(СтрокаТабличнойЧасти, ИмяРеквизитаСуммы, ИмяРеквизитаЦены, ДокументОбъект, СпособРасчета = Неопределено) Экспорт

	ИмяТабличнойЧасти = ОбщегоНазначения.ПолучитьИмяТабличнойЧастиПоСсылкеНаСтроку(СтрокаТабличнойЧасти);

	МетаданныеДокумента = ДокументОбъект.Метаданные();

	Сумма = СтрокаТабличнойЧасти[ИмяРеквизитаЦены] * СтрокаТабличнойЧасти.Количество;
	СуммаСкидки = 0;
	...
	СтрокаТабличнойЧасти[ИмяРеквизитаСуммы] = Сумма - СуммаСкидки;

КонецПроцедуры

// Расчет, исходя из постоянной суммы(Передаваемый реквизит) - (03.03.2016 - Кузнецов С.А)  
//
// Параметры:
//  СтрокаТабличнойЧасти - строка табличной части документа,
//  ИмяРеквизитаСуммы    - имя реквизита табличной части, которую необходимо рассчитать (Сумма)
//  ИмяРеквизитаСуммыНДС - имя реквизита табличной части, исходя из которой необходимо рассчитать сумму НДС (СуммаНДС)
//  ДокументОбъект       - объект редактируемого документа.
//
Процедура РассчитатьСуммуНДСТабЧастиПоИмениРеквизита(СтрокаТабличнойЧасти, ИмяРеквизитаСуммы, ИмяРеквизитаСуммыНДС, ДокументОбъект) Экспорт
	...
	СтрокаТабличнойЧасти[ИмяРеквизитаСуммыНДС] = УчетНДС.РассчитатьСуммуНДС(СтрокаТабличнойЧасти[ИмяРеквизитаСуммы],
	УчитыватьНДС, СуммаВключаетНДС,
	УчетНДС.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС));

КонецПроцедуры // РассчитатьСуммуНДСТабЧасти()

// Процедура выполняет стандартные действия при изменении модифицированной суммы - (03.03.2016 - Кузнецов С.А)  
// в строке табличной части документа.
//
// Параметры:
//  СтрокаТабличнойЧасти                             - строка табличной части документа,
//  ИмяРеквизитаСуммы                                - имя реквизита табличной части Суммы
//  ИмяРеквизитаЦены                                 - имя реквизита табличной части Цены
//  ДокументОбъект                                   - объект редактируемого документа,
//  ТекПользователь                                  - ссылка на текущего пользователя,
//  РасчетАвтоматическихСкидок                       - признак необходимости Расчета автоматических скидок,
//  ЕстьРеквизитДокументаПроцентСкидкиНаценки        - признак наличия в табличной части документа процента скидки (наценки), 
...
//  ИмяТабЧастиДокумента                             - имя табличной части.
//
Процедура ПриИзмененииСуммыТабЧастиПоИмениРеквизита(СтрокаТабличнойЧасти, ИмяРеквизитаСуммы, ИмяРеквизитаЦены, ДокументОбъект, ТекПользователь, РасчетАвтоматическихСкидок = Ложь, 
                                    ЕстьРеквизитДокументаПроцентСкидкиНаценки = Неопределено, 
                                    ПересчитыватьСкидкуДокумента = Неопределено, 
                                    ЕстьРеквизитДокументаПроцентАвтоматическихСкидок = Неопределено, 
                                    ИмяТабЧастиДокумента = Неопределено) Экспорт
	...
	Если СтрокаТабличнойЧасти.Количество=0 Тогда
		СтрокаТабличнойЧасти[ИмяРеквизитаЦены] = 0;
	Иначе	
		МетаданныеДокумента = ДокументОбъект.Метаданные();
		ЕстьРеквизитПроцентСкидкиНаценки = ?(ЕстьРеквизитДокументаПроцентСкидкиНаценки = Неопределено,
		                                      ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("ПроцентСкидкиНаценки", МетаданныеДокумента, 
		...
		Если ЕстьРеквизитПроцентСкидкиНаценки Тогда
			...
			Если ПересчитыватьСкидку Тогда
				Если (СтрокаТабличнойЧасти[ИмяРеквизитаЦены] = 0) Или (СтрокаТабличнойЧасти.Количество = 0) Тогда
					СтрокаТабличнойЧасти.ПроцентСкидкиНаценки = 0;
					Если СтрокаТабличнойЧасти[ИмяРеквизитаЦены] = 0 Тогда
						ОбщегоНазначения.СообщитьОбОшибке(ИмяРеквизитаЦены + " равна 0, после изменения суммы установлена нулевая скидка!");
					Иначе
						ОбщегоНазначения.СообщитьОбОшибке("Количество равно 0, после изменения суммы установлена нулевая скидка!");
					КонецЕсли;
				Иначе
					СуммаСоСкидками = СтрокаТабличнойЧасти[ИмяРеквизитаСуммы];
					СуммаБезСкидок  = СтрокаТабличнойЧасти[ИмяРеквизитаЦены] * СтрокаТабличнойЧасти.Количество;
					ПроцентСкидки   = 100 - (СуммаСоСкидками * 100) / СуммаБезСкидок;
					ПроцентАвтоматическихСкидок = 0;
			...
			Иначе
				...
				Иначе
					...
					Если ЕстьРеквизитПроцентАвтоматическихСкидок Тогда
						ПроцентАвтоматическихСкидок = СтрокаТабличнойЧасти.ПроцентАвтоматическихСкидок;
					КонецЕсли;
					СуммаБезСкидок  = СтрокаТабличнойЧасти[ИмяРеквизитаСуммы] * 100/
					                  (100 - СтрокаТабличнойЧасти.ПроцентСкидкиНаценки - ПроцентАвтоматическихСкидок);
					СтрокаТабличнойЧасти[ИмяРеквизитаЦены] = СуммаБезСкидок / СтрокаТабличнойЧасти.Количество;
				КонецЕсли;
			КонецЕсли;
		Иначе
			СтрокаТабличнойЧасти[ИмяРеквизитаЦены] = СтрокаТабличнойЧасти[ИмяРеквизитаСуммы] / СтрокаТабличнойЧасти.Количество;
		КонецЕсли;
	КонецЕсли;

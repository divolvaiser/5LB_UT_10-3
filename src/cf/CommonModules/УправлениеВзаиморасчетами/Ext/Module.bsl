...
Процедура ВыполнитьДвиженияПоРегистрамУпрВзаиморасчетов(ДокументОбъект, СтруктураШапкиДокумента, 
	...
	Если ТаблицаПоВзаиморасчетам.Количество() > 0 Тогда
		...
		Если (ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг")) И (ЗначениеЗаполнено(ДокументОбъект.Сделка)) Тогда
			Если НЕ ЗначениеЗаполнено(ДокументОбъект.Сделка.УдалитьКонтактноеЛицо) Тогда
				возврат;
			КонецЕсли;
			
			// По регистру ВзаиморасчетыСКонтрагентами
			
			НаборДвиженийВзаиморасчеты2 = ДокументОбъект.Движения.ВзаиморасчетыСКонтрагентами;
			
			ТаблицаДвижений2 = НаборДвиженийВзаиморасчеты2.Выгрузить();
			ТаблицаДвижений2.Очистить();
			
			ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(ТаблицаПоВзаиморасчетам, ТаблицаДвижений2);
			
			ТаблицаДвижений2.ЗаполнитьЗначения(СтруктураШапкиДокумента.Организация, "Организация");
			ТаблицаДвижений2.ЗаполнитьЗначения(ДокументОбъект.Сделка.УдалитьКонтактноеЛицо, "Контрагент");
			ТаблицаДвижений2.ЗаполнитьЗначения(ДокументОбъект.Сделка.УдалитьКонтактноеЛицо.ОсновнойДоговорКонтрагента, "ДоговорКонтрагента");
			
			НаборДвиженийВзаиморасчеты2.мПериод          = СтруктураШапкиДокумента.Дата;
			НаборДвиженийВзаиморасчеты2.мТаблицаДвижений = ТаблицаДвижений2;
			...
			Если Не Отказ Тогда
				ОбщегоНазначения.ВыполнитьДвижениеПоРегистру(НаборДвиженийВзаиморасчеты2, ВидДвижения);
			КонецЕсли;
		КонецЕсли;

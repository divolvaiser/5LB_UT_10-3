...
//07082017 Контроль отрицательных остатков в поступлениях - письма. Рустам. 
Процедура ОбработкаОчередиПисем() Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 5
	               ...
	               |	Рег.ТелоПисьма
	               |ИЗ
	               |	РегистрСведений.ОчередьПисем КАК Рег
	               |ГДЕ
	               |	НЕ Рег.Отправлено";
   ...
   //{+НАЧАЛО+Изменения} Автор: Тасмаджиев 23/05/2018//=============================================================================================	
	...
	//Профиль.ПортSMTP = 25;
	//Профиль.ПользовательSMTP = "1cinfo@ukitgroup.ru"; //НМА 16.01.18 изменить настройки почтового ящика. Старый ящик:"1cinfo@5lb.ru". Задача от Ивана Иванова
	//Профиль.ПарольSMTP = "!cDe$ert";
  ...
  //{-КОНЕЦ-Изменения} Автор: Тасмаджиев 23/05/2018//==============================================================================================
	...
	Пока Выб.Следующий() Цикл
		...
		Письмо.Тема = Выб.Тема;
		Письмо.Тексты.Добавить(Выб.ТелоПисьма);
		Письмо.Отправитель = "1cinfo@ukitgroup.ru"; //НМА 16.01.18 изменить настройки почтового ящика. Старый ящик:"1cinfo@5lb.ru". Задача от Ивана Иванова
		Письмо.ИмяОтправителя = "1cinfo";
		Письмо.Получатели.Добавить(Выб.Получатель);
		
		Попытка
			ИнтернетПочта.Послать(Письмо);
			
			Набор = РегистрыСведений.ОчередьПисем.СоздатьНаборЗаписей();
			Набор.Отбор.Получатель.Установить(Выб.Получатель);
			Набор.Отбор.Период.Установить(Выб.Период);

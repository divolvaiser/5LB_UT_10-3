...
Процедура УстановитьКартинкиВСтрокахТЧ_штрихи(ОформленияСтрок) Экспорт
	...
	КартинкаСерийныеНомера = БиблиотекаКартинок.НовыйПоШтрихкоду;
	Для каждого ОформлениеСтроки Из ОформленияСтрок Цикл
		
		НайденнаяСтрока = ТаблицаНоменклатуры.Найти(ОформлениеСтроки.ДанныеСтроки.Номенклатура, "Номенклатура");
		НайденнаяСтрока2 = ТаблицаНоменклатуры2.Найти(ОформлениеСтроки.ДанныеСтроки.Номенклатура, "Номенклатура");
		Если НайденнаяСтрока <> Неопределено Тогда
			ОформлениеСтроки.Ячейки.Номенклатура.УстановитьКартинку(КартинкаСерийныеНомера);
...
Функция ПроверитьМодифицированность(Документ) Экспорт
	...
	Если Документ.Модифицированность() Тогда
		...
		Предупреждение ("Документ изменен. Для печати его необходимо записать");
...
Процедура ПоказатьКодАртикул2(КолонкиТабПоля, ЯчейкиСтрокиТабПоля, Номенклатура) Экспорт
	...
	если КолонкиТабПоля.Найти("АртикулТовара") <> неопределено тогда
		если КолонкиТабПоля.АртикулТовара.Видимость тогда
			Если Номенклатура <> Неопределено Тогда
				ЯчейкиСтрокиТабПоля.АртикулТовара.УстановитьТекст(Номенклатура.Артикул);
			Иначе
				ЯчейкиСтрокиТабПоля.АртикулТовара.УстановитьТекст("");
			КонецЕсли;
		конецесли;
	конецесли;
...
// Процедура открывает форму обработки "Выгрузка данных в конфигурацию "Бухгалтерия предприятия", ред 1.5"
// для выгрузки в базовую версию.
Процедура ОткрытьФормуОбработкиВыгрузкаДанныхВ1СБухгалтерию81_15ВБазовуюВерсию() Экспорт
	
	ОбработкаОбъект = Обработки.ВыгрузкаДанныхВ1СБухгалтерию81_15.Создать();
...
Функция ПроверитьЧтоКачествоВСтруктуреПодбораНеУдовлетворяетУсловиямПодбора(СтруктураПодбора, КачествоМожетБытьТолькоНовым) Экспорт
	...
	Если КачествоМожетБытьТолькоНовым Тогда
		...
		Иначе
			...
			Предупреждение("Добавьте обычный товар, а после уцените его с помощью кнопки ""Уценка"".");
			Результат = Истина;
		КонецЕсли;
	Иначе
		Результат = Ложь;
...
Процедура ОбработатьВыборТаблицаОстатков(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка, ТекущаяНоменклатура) Экспорт
	...
	ИначеЕсли Колонка.Имя = "ВРезерве"  Тогда
		Если ТипЗнч(ВыбраннаяСтрока)=Тип("Массив") Тогда
			Если ЗначениеЗаполнено(ВыбраннаяСтрока[0].ВРезерве) Тогда
				Отчет = Отчеты.ОтчетСписокКроссТаблица.Создать();
				...
				СоответствиеОтбора = Новый Соответствие;
				СоответствиеОтбора.Вставить("Номенклатура", ТекущаяНоменклатура);
				Если ВыбраннаяСтрока[0].Родитель = Неопределено Тогда
					СоответствиеОтбора.Вставить("ХарактеристикаНоменклатуры", ВыбраннаяСтрока[0].ХарактеристикаСклад);
				Иначе
					СоответствиеОтбора.Вставить("ХарактеристикаНоменклатуры", ВыбраннаяСтрока[0].Родитель.ХарактеристикаСклад);
					СоответствиеОтбора.Вставить("Склад", ВыбраннаяСтрока[0].ХарактеристикаСклад);
				КонецЕсли; 
				СтруктураНостроек.Вставить("Отбор",СоответствиеОтбора);
				
				СоответствиеГруппировок = Новый Соответствие;
				СоответствиеГруппировок.Вставить("ДокументРезерва");
				СоответствиеГруппировок.Вставить("Перемещение");
				//СоответствиеГруппировок.Вставить("ДокументРезерва.СтатусыСборкиЗаказа");
				СтруктураНостроек.Вставить("ГруппировкиСтрок", СоответствиеГруппировок);
				
				СоответствиеПолей= Новый Соответствие;
				//СоответствиеПолей.Вставить("ДокументРезерва.ДатаОтгрузки");
				СоответствиеПолей.Вставить("ДокументРезерва.СтатусыСборкиЗаказа");
				СоответствиеПолей.Вставить("Перемещение.СтатусСборки");
		...
		ИначеЕсли ЗначениеЗаполнено(ВыбраннаяСтрока.ВРезерве) Тогда

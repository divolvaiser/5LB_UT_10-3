...
Процедура ПередЗаписьюДокументаПроверкаДоступностиПериода(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	// 16.08.17 Вялов - по согласованию с Д.Грухом - исключить из проверки закрытого периода >
		ОбщийМодуль5ЛБ.ПроверитьПравоНаРедактированиеДокументовТовародвижения(Источник, Отказ);//24.07.2017, Юра		
...
Процедура ПередЗаписьюРегистраДатаЗапретаРедактированияПередЗаписью(Источник, Отказ, Замещение) Экспорт
	//26.09.2017, Юра
	//20.12.2017 Учётная политика не сохраняется... . 
	Попытка
	...
	Если ДокРегистратор<>Неопределено Тогда
		Если НачалоДня(ДокРегистратор.Дата) = НачалоДня(ТекущаяДата()) И ТипЗНЧ(ДокРегистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			//в этом случае не будем ничего проверять
			//Мы просто пытаемся увеличить скорость проведения документа и печати чека ККМ для розничных продаж (тут они так делаются)
		Иначе
			//Было до 26.09.2017, Юра

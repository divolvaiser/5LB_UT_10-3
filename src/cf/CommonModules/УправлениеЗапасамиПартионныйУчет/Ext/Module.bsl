...
Функция ПолучитьДеревоПартийНаСкладахУпр (МоментКон, СтруктураПараметров)
	...
	//Возврат Запрос.Выполнить().Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам);
	 Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ" + Сред(Запрос.Текст, 8);
	 Попытка
		РезЗапрос = Запрос.Выполнить();
	 Исключение
		Сообщить("Ошибка при выполнении операции получения партий товаров на складах (упр.)!");		 
		Возврат Неопределено; 
	 КонецПопытки;
	 Попытка
		РезОперации = РезЗапрос.Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам);
		Возврат РезОперации;
	 Исключение
		Возврат Неопределено; 
	 КонецПопытки;

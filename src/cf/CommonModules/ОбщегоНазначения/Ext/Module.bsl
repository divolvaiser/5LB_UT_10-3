...
Процедура ПриУстановкеНовогоНомераДокументаПриУстановкеНовогоНомера(Источник, СтандартнаяОбработка, Префикс) Экспорт
	...
	//27.09.2017, Юра
	//Поскольку нигде в коде реквизит "НомерМагазина" не используется, значит не нужен.
	//Убрал этот запрос потому, что на поиск этого значения в тестовой базе уходит 2,5 минуты
	////Отдельная нумерация реализаций для магазинов
	//Если (ТипЗнч(Источник) = Тип("ДокументОбъект.РеализацияТоваровУслуг")) И (Источник.Склад.Родитель = Справочники.Склады.НайтиПоКоду("000000008")) Тогда
	//	Запрос = Новый Запрос;
	//	Запрос.Текст = "ВЫБРАТЬ
	//	|	МАКСИМУМ(Д.НомерМагазина) КАК НомерМагазина
	//	|ИЗ
	//	|	Документ.РеализацияТоваровУслуг КАК Д
	//	|ГДЕ
	//	|	Д.Дата МЕЖДУ &Дат1 И &Дат2
	//	|	И Д.Склад = &Склад";
	//	Запрос.УстановитьПараметр("Дат1",НачалоГода(ТекущаяДата()));
	//	Запрос.УстановитьПараметр("Дат2",КонецГода(ТекущаяДата()));
	//	Запрос.УстановитьПараметр("Склад", Источник.Склад);
	//	
	//	Рез = Запрос.Выполнить();
	//	Если Рез.Пустой() Тогда
	//		Источник.НомерМагазина = 1;
	//	Иначе
	//		Выб = Рез.Выбрать();
	//		Выб.Следующий();
	//		Если Выб.НомерМагазина = Null Тогда
	//			 Источник.НомерМагазина = 1;
	//		Иначе
	//		Источник.НомерМагазина = Выб.НомерМагазина + 1;
	//		КонецЕсли;
	//	КонецЕсли;
	//КонецЕсли;
КонецПроцедуры
...
КонецФункции  //***{-Добавления}*** Тасмаджиев 19/07/2018// <<-

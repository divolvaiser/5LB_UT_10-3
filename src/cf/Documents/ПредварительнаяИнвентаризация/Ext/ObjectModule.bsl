...
Функция ПолучитьДанныеДляПечатиДокумента() Экспорт
	...
	|	)
	|ИЗ
	|	Документ.ПредварительнаяИнвентаризация КАК ИнвентаризацияТоваровНаСкладе
	|
	|ГДЕ
...
#КонецЕсли
	...
	Если Склад.ВидСклада <> Перечисления.ВидыСкладов.НТТ Тогда
		...
		Для Каждого ст из Товары Цикл
			СписокНом.Добавить(ст.Номенклатура);
		КонецЦикла;
...
Процедура ЗаполнитьПоЗаказам (Датко) Экспорт
	...
	Запрос.Текст="ВЫБРАТЬ
	             ...
	             |	Товары.Коэффициент
	             |ИЗ
	             |	РегистрСведений.ИсторияСтатусовДокументов КАК Рег
	             |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказПокупателя.Товары КАК Товары
	             |		ПО Рег.Документ = Товары.Ссылка
	             |			И (Рег.Статус В (&Списак))
	             |			И (Рег.ДатаСтатуса = &Дата)
	             |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПокупателей.Остатки(&Дата2, ) КАК ЗаказыОстатки
	             |		ПО (ЗаказыОстатки.ЗаказПокупателя = Рег.Документ)
	             |ГДЕ
	             ...
	             |	Товары.ЕдиницаИзмерения,
	             |	Товары.Коэффициент";
	Запрос.УстановитьПараметр("Дата",Датко);
	Запрос.УстановитьПараметр("Дата2",КонецДня(Датко));
	Запрос.УстановитьПараметр("Списак",СписАк.ВыгрузитьЗначения());

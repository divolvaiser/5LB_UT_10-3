...
Функция ПечатьЦенников()
	...
	ОбработкаПечатьЦенников.ТипЦен = ЭтотОбъект.СкладОрдер.ТипЦенРозничнойТорговли;
	
	Отбор = Новый Структура("Объект");
	Отбор.Объект = ЭтотОбъект.СкладОрдер.Ссылка; 
	ВыбРег = РегистрыСведений.ЗначенияСвойствОбъектов.Выбрать(Отбор);
	Пока ВыбРег.Следующий() Цикл
		Если СокрЛП(ВыбРег.Свойство.Наименование) = СокрЛП(ЭтотОбъект.СкладОрдер.Наименование)	Тогда
...
Функция ПечатаемНашиШтрихиИПустые()
	...
	Для каждого ст Из Товары Цикл
		МассивНоменклатуры.Добавить(ст.Номенклатура);
	КонецЦикла;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Р.Владелец КАК Номенклатура
	|ИЗ
	|	РегистрСведений.Штрихкоды КАК Р
	|ГДЕ
	|	Р.Штрихкод ПОДОБНО ""200%""
	|	И Р.Владелец В(&МассивНоменклатуры)";
	...
	|	РегистрСведений.Штрихкоды КАК Р
	|ГДЕ
	|	Р.Владелец В(&МассивНоменклатуры)";
	
	ТаблицаНоменклатуры2 = Запрос.Выполнить().Выгрузить();
	ТаблицаНоменклатуры2.Индексы.Добавить("Номенклатура");
	
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПоступлениеТоваровУслуг_Штрихи";
	Макет       = ПолучитьМакет("НашиШтрихиИБез");
	
	ОбластьМакета = Макет.ПолучитьОбласть("Шапко");
	...
	Для каждого стр Из Товары Цикл
		...
		НайденнаяСтрока = ТаблицаНоменклатуры.Найти(стр.Номенклатура, "Номенклатура");
		НайденнаяСтрока2 = ТаблицаНоменклатуры2.Найти(стр.Номенклатура, "Номенклатура");
		Если НайденнаяСтрока <> Неопределено Тогда
			ОбластьМакета.Параметры.НомерСтроки = стр.НомерСтроки;
			ОбластьМакета.Параметры.Товар       = стр.Номенклатура.Наименование;
			ОбластьМакета.Параметры.Количество = стр.Количество;
			ОбластьМакета.Параметры.ЕдиницаИзмерения = стр.ЕдиницаИзмерения;
			Если ТекстКодАртикул = "Артикул" Тогда
				ОбластьМакета.Параметры.КодАртикул = стр.Номенклатура.Артикул;
			Иначе
				ОбластьМакета.Параметры.КодАртикул = стр.Номенклатура.Код;
			КонецЕсли;
			
			ТабДокумент.Вывести(ОбластьМакета);
			
		ИначеЕсли НайденнаяСтрока2 = Неопределено Тогда
			ОбластьМакета.Параметры.НомерСтроки = стр.НомерСтроки;
			ОбластьМакета.Параметры.Товар       = стр.Номенклатура.Наименование;
			ОбластьМакета.Параметры.Количество = стр.Количество;
			ОбластьМакета.Параметры.ЕдиницаИзмерения = стр.ЕдиницаИзмерения;
...
Функция АктПриёмки()
	...
	ЗапросПоТоварам.УстановитьПараметр("Кор", Справочники.КлассификаторЕдиницИзмерения.НайтиПоНаименованию("кор"));
	...
	|   ВЫБОР КОГДА ЕСТЬNULL(ВложенныйЗапрос.Штрих,0)=0 ИЛИ ВложенныйЗапрос.Штрих=0 Тогда
	|    ПРЕДСТАВЛЕНИЕ(""Добавить"") 
	|   Когда ВложенныйЗапрос.Штрихкод_200 = 1 Тогда
	|    ПРЕДСТАВЛЕНИЕ(""Наш"") КОНЕЦ КАК Штрих,
	|    ВложенныйЗапрос.Коэфф,
	|    ВложенныйЗапрос.Ширина КАК Ширина,
	|    ВложенныйЗапрос.Высота КАК Высота,
	|    ВложенныйЗапрос.Глубина КАК Глубина,
	|    ВложенныйЗапрос.Вес
	|ИЗ
	|	(ВЫБРАТЬ
	|		ПоступлениеТоваровУслуг.Номенклатура КАК Номенклатура,
	|		ПоступлениеТоваровУслуг.Номенклатура.Вес КАК Вес,
	|       КОЛИЧЕСТВО(Штрих.Штрихкод) КАК Штрих,
	|       Выбор Когда ШтрихКод_200.Штрихкод Есть Null Тогда 0 Иначе 1 КОНЕЦ КАК Штрихкод_200,
	|       Единицы.Коэффициент КАК Коэфф,
	|       Единицы.Ширина КАК Ширина,
	|       Единицы.Высота КАК Высота,
	|       Единицы.Глубина КАК Глубина,
	...
	|       ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК Штрих 
	|       ПО Штрих.Владелец = ПоступлениеТоваровУслуг.Номенклатура
	|         И Штрих.ЕдиницаИзмерения = ПоступлениеТоваровУслуг.ЕдиницаИзмерения
	|       ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК ШтрихКод_200 
	|       ПО ШтрихКод_200.Владелец = ПоступлениеТоваровУслуг.Номенклатура
	|         И ШтрихКод_200.ЕдиницаИзмерения = ПоступлениеТоваровУслуг.ЕдиницаИзмерения
	|         И ШтрихКод_200.ШтрихКод ПОДОБНО ""200%""
	|       ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЕдиницыИзмерения КАК Единицы
	|       ПО Единицы.Владелец = ПоступлениеТоваровУслуг.Номенклатура
	|       И  Единицы.ЕдиницаПоКлассификатору = &Кор
	|	
	|	ГДЕ
	|		ПоступлениеТоваровУслуг.Ссылка = &ТекущийДокумент
	|	
	|	СГРУППИРОВАТЬ ПО
// < 19.09.16 Задача № 3755 Вялов
	|		ПоступлениеТоваровУслуг.НомерСтроки,
// 19.09.16 Задача № 3755 Вялов >
	|		ПоступлениеТоваровУслуг.Номенклатура,
	|       ПоступлениеТоваровУслуг.Номенклатура.Вес,
	|       Выбор Когда ШтрихКод_200.Штрихкод Есть Null Тогда 0 Иначе 1 КОНЕЦ,
	...
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПоступлениеТоваровУслуг_АктПриемки";
	Макет       = ПолучитьМакет("АктПриемки");
	...
	ОбластьМакета.Параметры.Дата = Формат(ТекущаяДата(),"ДЛФ=Д");
	ТабДокумент.Вывести(ОбластьМакета);
	...
	Для Каждого ВыборкаСтрокТовары Из ВыборкаСтрок Цикл
		...
		Если ЗначениеЗаполнено (ВыборкаСтрокТовары.Ширина) Тогда
			ОбластьМакета.Параметры.размеркор = Строка(ВыборкаСтрокТовары.Ширина)+"ш/";
		Иначе
			ОбластьМакета.Параметры.размеркор = "ш/";
		КонецЕсли;
		Если ЗначениеЗаполнено (ВыборкаСтрокТовары.Высота) Тогда
			ОбластьМакета.Параметры.размеркор = ОбластьМакета.Параметры.размеркор + Строка(ВыборкаСтрокТовары.Высота)+"в/";
		Иначе
			ОбластьМакета.Параметры.размеркор = ОбластьМакета.Параметры.размеркор + "           в/";
		КонецЕсли;
		Если ЗначениеЗаполнено (ВыборкаСтрокТовары.Глубина) Тогда
			ОбластьМакета.Параметры.размеркор = ОбластьМакета.Параметры.размеркор + Строка(ВыборкаСтрокТовары.Глубина)+"г/";
		Иначе
			ОбластьМакета.Параметры.размеркор = ОбластьМакета.Параметры.размеркор + "           г/";
		КонецЕсли;


		
	
		//ОбластьМакета.Параметры.размеркор = Строка(ВыборкаСтрокТовары.Ширина)+"ш/     "+Строка(ВыборкаСтрокТовары.Высота)+"в/     "+Строка(ВыборкаСтрокТовары.Глубина)+"г/"; 
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Если РольДоступна(Метаданные.Роли.КладOFFщик) Тогда
		СтруктураМакетов.Вставить("АктПриёмки",       "Акт приёмки товара");
	ИначеЕсли РольДоступна(Метаданные.Роли.МенеджерПоПродажам) Тогда
	...
	Иначе
		...
		СтруктураМакетов.Вставить("Ценники",         "Ценники на товары");
		СтруктураМакетов.Вставить("Штрихи",          "Наши штрих-коды + без штрих-кода");
	...
	Если РольДоступна(Метаданные.Роли.Цены) Тогда
		СтруктураМакетов.Вставить("АктПриёмки",      "Акт приёмки товара");
	КонецЕсли;
...
Процедура ЗаполнитьТоварыПоОснованиюРеализация(ДокументОснование) Экспорт
	...
	|	И Док.Ссылка ССЫЛКА Документ.РеализацияТоваровУслуг";
	...
	Пока Выборка.Следующий() Цикл
		...
		//================================================!{Для поступлений на основании реализаций для Розничных магазинов}!=====================================================
		Если ТипЗнч(ДокументОснование.Ссылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			
			Если ДокументОснование._5LB_Врозницу   Тогда
				
				СтрокаТабличнойЧасти.КоличествоПлан = Выборка.Количество;
				СтрокаТабличнойЧасти.СуммаИзДокумента = СтрокаТабличнойЧасти.Сумма;
				СтрокаТабличнойЧасти.ЦенаИзДокумента = СтрокаТабличнойЧасти.Цена; 
				//***{+Добавления}*** Тасмаджиев 09/08/2018// ->>
				//***=!{Для внутреннего документооборота}!=*** ->>
			ИначеЕсли ЗначениеЗаполнено(ДокументОснование.Ссылка._5LB_Магазин) и ДокументОснование.Ссылка._5LB_СтатусПеревозки  = Перечисления.СтатусПеревозки.ВПути Тогда
				...
				ОбработкаОбъектЗначенияСвойств.ОбъектОтбораЗначений =ДокументОснование._5LB_Магазин;
				ОбработкаОбъектЗначенияСвойств.ОбъектОтбораНазначений = ДокументОснование._5LB_Магазин; 
...
Функция ПодготовитьТаблицуТоваров(РезультатЗапросаПоТоварам, СтруктураШапкиДокумента)
	...
	////НМА 15.12.16 >> 
	//Если ЗначениеЗаполнено(EmeWmsНомер) Тогда
	//	//Оставляем только строки с качеством товара Новый
	//	ПараметрыОтбораСклада = Новый Структура("Склад",СкладОрдер);
	//	Если ТаблицаТоваров.Количество()>0 Тогда
	//		ТаблицаТоваров = ТаблицаТоваров.Скопировать(ПараметрыОтбораСклада);
	//	КонецЕсли;
	//КонецЕсли;
	////НМА 15.12.16 <<
...
Процедура ОбработкаЗаполнения(Основание)
	...
	//{+НАЧАЛО+Редактирования} Автор: Тасмаджиев 13/06/2018//=============================================================================================
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг") или ТипЗнч(Основание) = Тип("ДокументОбъект.РеализацияТоваровУслуг") Тогда
		//{-КОНЕЦ-Редактирования} Автор: Тасмаджиев 13/06/2018//==============================================================================================
		
		ВидПоступления = Перечисления.ВидыПоступленияТоваров.НаСклад;
		//{+НАЧАЛО+Добавления} Автор: Тасмаджиев 05/06/2018//=========================================================================================
		//================================================!{Для поступления в розничный магазин}!=====================================================
		Если Основание._5LB_Врозницу ИЛИ 
			ЗначениеЗаполнено(Основание._5LB_Магазин) и Основание._5LB_СтатусПеревозки  = Перечисления.СтатусПеревозки.ВПути И не Основание._5LB_Врозницу Тогда
			...
			//====================================================СКЛАДЫ==============================================================
			Если ЗначениеЗаполнено(Основание._5LB_Магазин) Тогда
				СкладОрдер = Основание._5LB_Магазин; 
			Иначе
				ВызватьИсключение НСтр("ru = 'Не заполнен реквизит Магазин в документе основании.'");
			КонецЕсли; 
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
		//{-КОНЕЦ-Добавления} Автор: Тасмаджиев 05/06/2018//==============================================================================================
		ЗаполнениеДокументов.ПриИзмененииЗначенияКонтрагента(ЭтотОбъект, мСтруктураПараметровДляПолученияДоговора);
		//#***+Добавления*** Тасмаджиев 10/09/2018// ->>
		//#*** ЗаполнениеДокументов.ПриИзмененииЗначенияКонтрагента() - очищает Сделку по этому присваиваем значение позже.***# ->>
		...
		// Катков А. 12.09.2016 >> "Заказы поставщику" планируется использовать только в связке с WMS
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки_УТ11") Тогда
		СтруктураПоиска = Новый Структура();
		СтруктураПоиска.Вставить("Решение",Перечисления._5LB_РешенияПоПретензионнымТоварам.Допоставка);
		...
		Запрос.УстановитьПараметр("Сделка",Основание.Ссылка);
		Результат = Запрос.Выполнить();
		
		Если Основание.Товары.НайтиСтроки(СтруктураПоиска).Количество()<> 0 Тогда  //Проверка  тч содержит необходимые строки
			
			Если Результат.Пустой() Тогда //Если поступление не создавалось , то создаем списание и поступление.
				ЗаполнениеДокументов.ЗаполнитьШапкуДокументаПоОснованию(ЭтотОбъект, Основание);
				ЭтотОбъект.Дата = ТекущаяДата();
				ЭтотОбъект.Сделка = Основание.Ссылка;
				ЗаполнитьТоварыПоАктуОрасхождениях(Основание, Ложь);
				//***=!{Документ списания с претензионного склада}!=*** ->>
				ДокументСписания = Документы.СписаниеТоваров.СоздатьДокумент();
				ЗаполнитьЗначенияСвойств(ДокументСписания,Основание);
				ДокументСписания.Склад =  ПолучитьПретензионныйСклад(Основание.СкладОрдер);
				ДокументСписания.Дата = ТекущаяДата();
				ДокументСписания.Номер = "";
				ДокументСписания.ИнвентаризацияТоваровНаСкладе = Основание.Ссылка;
				
				Для каждого Строка ИЗ Основание.Товары Цикл 
					
					Если Строка.Решение = Перечисления._5LB_РешенияПоПретензионнымТоварам.Допоставка Тогда
						СтрокаТабличнойЧасти = ДокументСписания.Товары.Добавить();
				...
				ДокументСписания.Записать(РежимЗаписиДокумента.Проведение,РежимПроведенияДокумента.Оперативный);
				
			Иначе //Заполняем Сделку для того чтобы при открытии 
				//формы, отказаться от открытия нового документа
				//(ТипЗнч(Сделка.Ссылка) = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки_УТ11"))
				//и открыть ранее созданный.
				ЭтотОбъект.Сделка = Основание.Ссылка;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = 
				" Данный документ уже был создан,
				|на основании Акта о расхождениях,
				|распроведите и пометте его на удаление 
				|для создания нового документа
				|или используйте этот.";
				Сообщение.Сообщить();
			КонецЕсли;
		Иначе
			//Заполняем Сделку для того чтобы при открытии 
			//формы, отказаться от открытия нового документа
			//(ТипЗнч(Сделка.Ссылка) = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки_УТ11"))
			//и открыть ранее созданный.
			ЭтотОбъект.Сделка = Основание.Ссылка;
			Возврат; //<--<--<--RETURN!<--<--<--RETURN!<--<--<--RETURN!<--<--<--RETURN!<--<--<--RETURN!<--<--<--	
		КонецЕсли;
		//***{-Добавления}*** Тасмаджиев 03/08/2018// <<-
		
		//#***+Добавления*** Тасмаджиев 23/11/2018// ->>
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка._5LB_ПлановоеПоступление") или ТипЗнч(Основание) = Тип("ДокументОбъект._5LB_ПлановоеПоступление") Тогда
		
		
		СтатусПоступленияТоваров = Перечисления.СтатусыПоступленийТоваровИУслуг.КПоступлению;
		ВидПоступления = Перечисления.ВидыПоступленияТоваров.НаСклад;
		СкладОрдер		= Основание.СкладРеализации;
		ЗаполнениеДокументов.ПриИзмененииЗначенияКонтрагента(ЭтотОбъект, мСтруктураПараметровДляПолученияДоговора);
		Сделка = Основание.Ссылка;
		...
		|	ЗаказПокупателяТовары.Сумма КАК СуммаИзДокумента,
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	//08082017 Контроль отрицательных остатков в поступлениях. Рустам. 
	Перем лОтказ;
	...
	Если НЕ РольДоступна(Метаданные.Роли.ПолныеПрава) Тогда
		Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
			Если ПроблемаСЦеной() Тогда
				#Если Клиент Тогда
					Сообщить ("Есть цены выше установленных. Выделены красным",СтатусСообщения.Важное);
				#КонецЕсли	
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
	...
	//08082017 Контроль отрицательных остатков в поступлениях. Рустам. 
	лОтказ = Ложь;
	РаботаСПоступлениями.КонтрольПроведенияПоступленияОбработкаПроведения(ЭтотОбъект,лОтказ,РежимПроведения);
	Если лОтказ Тогда
		РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения;
		Сообщить ("Не удалось провести документ",СтатусСообщения.Внимание);
		возврат;
	КонецЕсли;
	//08082017
	...
	КурсДокументаБух = Товары.Итог("СуммаИзДокумента");
	...
	////НМА 01.07.17 >>
	//Если ЭтоНовый() Тогда
	//	СтатусПоступленияТоваров = Перечисления.СтатусыПоступленийТоваровИУслуг.Новый;
	//	EmeWmsНомер = "";
	//КонецЕсли;
	////НМА 01.07.17 <<
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//++ Кузнецов С.А. - Задача в МегаПлане "3249" (Закомментировал, переносим "ПриЗаписи") - 27.04.2016
	//++ Кузнецов С.А. - Добавим/изменим данные о номенклатуре поставщика - 28.03.2016
	//Если Не Отказ Тогда
	//	РегистрыСведений.СоответствиеНоменклатурыСНоменклатуройПоставщика.ОбновлениеДанныхОНоменклатуреПоставщика(Ссылка);
	//КонецЕсли;	 
	//-- Кузнецов С.А. - 28.03.2016	 	
...
Процедура ПриЗаписи(Отказ)
	...
	//++ Кузнецов С.А. - Задача в МегаПлане "3249" (переносим "ПриЗаписи") - 27.04.2016
	Если Не Отказ Тогда
		РегистрыСведений.СоответствиеНоменклатурыСНоменклатуройПоставщика.ОбновлениеДанныхОНоменклатуреПоставщика(Ссылка);
	КонецЕсли;	 
	//-- Кузнецов С.А. - 27.04.2016
	...
	Если ОбменДанными.Загрузка  Тогда
		Возврат;
	Иначе
		//***{+Редактирования}*** Тасмаджиев 10/08/2018// ->>
		//***=!{Документооборот , было перенесено в подписку "Припроведении поступления" т.к условия поменялись. (ЗКОММЕНТИРОВАННОЕ УДАЛЯТЬ МОЖНО ЧЕРЕЗ МЕСЯЦ)}!=*** ->>
		////{+НАЧАЛО+Добавления} Автор: Тасмаджиев 09/06/2018//==============================================================================================
		////15062018 Только на клиенте должно работать
		//#Если Клиент Тогда  
		...
		//	Иначе
		//		Если ТипЗнч(ДокументОснование.Ссылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")Тогда
		//			//================================================!{РеализацияТоваровУслуг Изменение}!=====================================================
		//			РеализацияТоваровУслуг = ДокументОснование.ПолучитьОбъект();
		...
		//		//================================================!{заполняем колонку Количествопо колонке КоличествоПринято }!===================================
		...
		//		
		//		
		//		_5LB_ПриемкаТовараЗакончена = Истина;
		//		ЕстьРасхождения = ПроверитьРасхожденияКоличестваТоваров();
		//		
		...
		//			АктОРасхожденияхПослеПриемки.Заполнить(ЭтотОбъект);
...
//Контроль цен поставщиков
Функция ПроблемаСЦеной ()
	//Было до 11.10.2017, Юра
	//Запрос = Новый Запрос;
	...
	//               |	&Табл КАК Таб
	//               |ГДЕ
	//			   |    Таб.ЦенаПоставщика > 0
	//               |	И Таб.Цена > Таб.ЦенаПоставщика
	//               |;
	//               |
	//               |////////////////////////////////////////////////////////////////////////////////
	//               |ВЫБРАТЬ
	...
	//               |ИЗ
	//               |	ТЗ КАК ТЗ";
	//Запрос.УстановитьПараметр("Табл",Товары.Выгрузить());
	...
	//Стало после 11.10.2017, Юра
	Для Каждого Строка Из Товары Цикл
		Если Строка.ЦенаПоставщика > 0 И Строка.Цена > Строка.ЦенаПоставщика Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
	Возврат Ложь;
...
Функция ПроверитьРасхожденияКоличестваТоваров()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
...
Процедура ЗаполнитьТоварыПоАктуОрасхождениях(ДокументОснование, ПодтверждатьОчисткуСтрок) Экспорт
	...
	Для каждого Строка ИЗ ДокументОснование.Товары Цикл 
		...
		Если Строка.Решение = Перечисления._5LB_РешенияПоПретензионнымТоварам.Допоставка Тогда
			...
			ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти,Строка);
			ОбработкаТабличныхЧастей.РассчитатьКоличествоМестТабЧасти(СтрокаТабличнойЧасти, ЭтотОбъект);
			СтрокаТабличнойЧасти.КоличествоПлан = СтрокаТабличнойЧасти.Количество;
			//СтрокаТабличнойЧасти.Цена = ?(Строка.Цена>0,Строка.Цена,НайтиНеНайденныйСебест (Строка.Номенклатура));
			ОбработкаТабличныхЧастей.РассчитатьСуммуТабЧасти(СтрокаТабличнойЧасти, ЭтотОбъект);
...
//***{+Добавления}*** Тасмаджиев 02/08/2018// ->>
...
КонецФункции //***{-Добавления}*** Тасмаджиев 02/08/2018// <<-

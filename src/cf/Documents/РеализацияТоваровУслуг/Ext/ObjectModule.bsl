...
//{+НАЧАЛО+Добавления} Автор: Тасмаджиев 04/07/2018//==============================================================================================
Если ЗначениеЗаполнено(ЭтотОбъект.Сделка) Тогда
	Если ЭтотОбъект.Сделка.СтатусыСборкиЗаказа = Перечисления.СтатусыСборкиЗаказа.Собран Тогда
	
		Если РольДоступна("_5LB_Перемещение_товаров_склад") Тогда
			Объ = ЭтотОбъект.Ссылка.ПолучитьОбъект();
			Объ._5LB_СтатусПеревозки = Перечисления.СтатусПеревозки.ВПути;
			Объ.Записать(РежимЗаписиДокумента.Запись);
		КонецЕсли;
		
		Если ЭтотОбъект.Склад = Справочники.Склады.НайтиПоКоду("000000005") Тогда
			//#***+Изменения*** Тасмаджиев 03/10/2018// ->>
			//или ЭтотОбъект.СкладОрдер = Справочники.Склады.НайтиПоКоду("550      ")Тогда
			//#***-Изменения*** Тасмаджиев 03/10/2018// <<-
	...
	КонецЕсли;
КонецЕсли;
	//{-КОНЕЦ-Добавления} Автор: Тасмаджиев 04/07/2018//==============================================================================================
...
//{+НАЧАЛО+Добавления} Автор: Тасмаджиев 12/07/2018//==============================================================================================
...
Функция ПечатьУниверсальногоПередаточногоДокумента()
	...
	СтруктураОтбораСчетаФактуры = Неопределено;
	...
	Если ЗначениеЗаполнено(СчетФактура) ИЛИ СчетФактураТребуется Тогда
		...
		РеквизитыСФ = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(СчетФактура,"Дата");
		...
		ДанныеДокумента.Вставить("ВидСчетаФактуры", 		Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию);
		...
		ДанныеДокумента.Вставить("СчетФактураБезНДС", 		ЗначениеЗаполнено(СчетФактура.СуммаНДС));
		ДанныеДокумента.Вставить("КППКонтрагента", 	 		СчетФактура.Контрагент.КПП);
		...
		СЧФПечатьУниверсальногоПередаточногоДокумента(ДанныеДляПечати, ТабДокумент);
		Возврат ТабДокумент;
...
//
Функция СЧФПечатьУниверсальногоПередаточногоДокумента(ДанныеДляПечати, ТабДокумент = Неопределено) Экспорт

	Если ТабДокумент = Неопределено Тогда
		ТабДокумент = Новый ТабличныйДокумент;
	КонецЕсли;
	
	Если Дата < НачалоДействияПостановления981() Тогда
		Макет = ПолучитьОбщийМакет("УниверсальныйПередаточныйДокумент");
		ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_УниверсальныйПередаточныйДокумент";
	Иначе
		Макет = ПолучитьОбщийМакет("УниверсальныйПередаточныйДокумент981");
...
Процедура ВывестиПодвалУниверсальногоПередаточногоДокументаВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
	...
	Если ДанныеДляПечати.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию Тогда
		...
		Если ТипЗнч(ДанныеДляПечати.Покупатель) = Тип("СправочникСсылка.Контрагенты") И ЗначениеЗаполнено(ДанныеДляПечати.Покупатель) Тогда
			...
			РеквизитыПокупателя = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ДанныеДляПечати.Покупатель, "ГоловнойКонтрагент");
			//Если РеквизитыПокупателя.ОбособленноеПодразделение И ЗначениеЗаполнено(РеквизитыПокупателя.ГоловнойКонтрагент) Тогда
			   Если  ЗначениеЗаполнено(РеквизитыПокупателя.ГоловнойКонтрагент) Тогда
...
Процедура ВывестиСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
     //{+НАЧАЛО+Добавления} Автор: Тасмаджиев 12/07/2018//==============================================================================================
	 //{+НАЧАЛО+Добавления} Автор: Тасмаджиев 03/07/2018//==============================================================================================
		
		Если ЗначениеЗаполнено(ЭтотОбъект._5LB_Магазин) и ЭтотОбъект._5LB_СтатусПеревозки  = Перечисления.СтатусПеревозки.ВПути И не ЭтотОбъект._5LB_Врозницу  Тогда
			
			ШтрихкодДляПечати = НайтиШтрихкод();
			
			Если ШтрихкодДляПечати  = Неопределено Тогда
				
				ШтрихкодДляПечати =  СоздатьШтрихкод();
				
			КонецЕсли;
			
		КонецЕсли;
		
	//{-КОНЕЦ-Добавления} Автор: Тасмаджиев 03/07/2018//==============================================================================================

	 //{-КОНЕЦ-Добавления} Автор: Тасмаджиев 12/07/2018//==============================================================================================
	...
	Если ДанныеДляПечати.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию Тогда
		...
		Если ТипЗнч(ДанныеДляПечати.Покупатель) = Тип("СправочникСсылка.Контрагенты") И ЗначениеЗаполнено(ДанныеДляПечати.Покупатель) Тогда
			//***{+Изменения}*** Тасмаджиев 09/08/2018// ->>
			//РеквизитыПокупателя = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ДанныеДляПечати.Покупатель, "ГоловнойКонтрагент, ОбособленноеПодразделение");
			РеквизитыПокупателя = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ДанныеДляПечати.Покупатель, "ГоловнойКонтрагент");
			//***{-Изменения}*** Тасмаджиев 09/08/2018// <<-
			Если  ЗначениеЗаполнено(РеквизитыПокупателя.ГоловнойКонтрагент) Тогда
	...
	КонецЕсли;
	 ...
	 //{+НАЧАЛО+Добавления} Автор: Тасмаджиев 05/07/2018//============================================================
	//================================================!{ШТРИХКОД}!====================================================
	Если ЗначениеЗаполнено(ШтрихкодДляПечати) Тогда
			попытка
				ОбластьМакета.Рисунки.D1.Объект.ТипКода = 1;
				ОбластьМакета.Рисунки.D1.Объект.Сообщение = ШтрихкодДляПечати;				
			исключение
				сообщить("ошибка печати штрихкода: "+ОписаниеОшибки());
			конецпопытки;		
		Иначе
			#Если Клиент Тогда
			...
			#КонецЕсли
	КонецЕсли;
	//{-КОНЕЦ-Добавления} Автор: Тасмаджиев 05/07/2018//=================================================================
	...
	Для Каждого Строчка Из ВыборкаСтрокТовары Цикл
		...
		Если ВыборкаСтрокТовары.Колонки.Найти("ТоварКод")<> Неопределено Тогда 
			
		    Строчка.ТоварКод = ?(ВыводитьКоды, Строчка.Товар.Артикул, "");
	...
	ВыборкаСтрокТовары.Свернуть("Товар, ТоварНаименование, ЕдиницаИзмерения, Цена,СтавкаНДС,НомерГТД, ПредставлениеГТД, СтранаПроисхождения, ПредставлениеСтраны",
...
Функция НайтиШтрихкод()  Экспорт
	...
	|ГДЕ
	|	Р.Владелец = &Док
	|	И Р.Владелец ССЫЛКА Документ.ПоступлениеТоваровУслуг";
...
Функция СоздатьШтрихкод() Экспорт
	#Если Клиент Тогда
...
Функция ПолучитьМассивСтрокSailPlayВЧек() Экспорт
	//08.02.2017, Юра
	Если НЕ ТекущийРежимЗапуска() = РежимЗапускаКлиентскогоПриложения.ОбычноеПриложение Тогда
		ECR = Неопределено;
	КонецЕсли;
		
    МассивСтрокSP = Новый Массив;
    Попытка
        НеподтвержденныхБонусовSailPlay = НеподтвержденныхБонусовSailPlay;    	
        ДоступноБонусовSailPlay         = ДоступноБонусовSailPlay;
    Исключение
        Возврат МассивСтрокSP;
    КонецПопытки; 
    ...
    ШиринаЛенты = ECR.ЧЛДлинаСтрокиСимволов;
    
    тАкции = "АКЦИИ";
    Дл = ШиринаЛенты - СтрДлина(тАкции);
    ДобавитьЗнаков = ОКР(Дл/2-1,0);
    стрДобавить = "";
    Для сч=1 По ДобавитьЗнаков Цикл
        стрДобавить = "-" + стрДобавить;
    КонецЦикла; 
    тАкции = стрДобавить + тАкции + стрДобавить;
    МассивСтрокSP.Добавить(тАкции);
    тАкции = СокрЛП(ПримененныеАкцииSailPlay);
	Если НЕ тАкции = "" Тогда
		Если ШиринаЛенты > 0 Тогда
			сч = 1;
			Пока сч <=20 Цикл  //ограничу печать 20-ю строками
				СтрокаПечати = СокрЛП(Лев(тАкции,ШиринаЛенты));
				МассивСтрокSP.Добавить(СтрокаПечати);
				тАкции = СокрЛП(СтрЗаменить(тАкции,СтрокаПечати,""));
				Если тАкции = "" Тогда
					сч = 21;
					Прервать;
				КонецЕсли;
		...
		КонецЕсли;	
	Иначе	
		//++ Кузнецов С.А. - Не выводим пустую строку "-----АКЦИИ-----" в чеке - 29.06.2016
		МассивСтрокSP.Очистить();
		//-- Кузнецов С.А. - 29.06.2016    
	КонецЕсли; 
    
    ////печатаем бонусы
    Если НомерКартыSailPlay <> "" ИЛИ НомерТелефонаSailPlay <> "" Тогда
        СтрокаБонусы = "Бонусы";
        КоличествоЗвездСКаждойСтороны = ОКР((ШиринаЛенты - СтрДлина(СтрокаБонусы))/2,0);

        СтрокаЗвезды ="";
        ДЛя  сч = 1 ПО КоличествоЗвездСКаждойСтороны Цикл
            СтрокаЗвезды = СтрокаЗвезды + "*";
        КонецЦикла;
        
        СписатьБонусов = Товары.Итог("СуммаСкидкиОплатыБонусомSailPlay");
        мДоступноБонусовSailPlay  			= ДоступноБонусовSailPlay - СписатьБонусов;
        ЗаПокупкуБонусовSailPlay  		    = БонусовНачисленоSailPlay;
        
        мНеподтвержденныхБонусовSailPlay = НеподтвержденныхБонусовSailPlay + ЗаПокупкуБонусовSailPlay;
        РамкаБонусы = СтрокаЗвезды + СтрокаБонусы + СтрокаЗвезды;
        МассивСтрокSP.Добавить("  ");
        МассивСтрокSP.Добавить(РамкаБонусы);
        МассивСтрокSP.Добавить("Подтвержденных: " + СокрЛП(мДоступноБонусовSailPlay));
        МассивСтрокSP.Добавить("Неподтвержденных: " + СокрЛП(мНеподтвержденныхБонусовSailPlay));
        МассивСтрокSP.Добавить("Будет начислено: " + СокрЛП(ЗаПокупкуБонусовSailPlay));
        
        Бонус = Формат(СписатьБонусов, "ЧЦ=15; ЧДЦ=2; ЧРД=.; ЧН=; ЧГ=0");
        стрСкидки = "Оплачено бонусами: ";
        стрПечать = стрСкидки + СокрЛП(Бонус);
        МассивСтрокSP.Добавить(стрПечать);
...
// ins SailPlay 04.07.2016 
Процедура ОбнулитьСкидкиSailPlay() Экспорт
    Если ПараметрыСеанса.НеВыгружатьВSailPlay = Ложь
        И SailPlay_СкидкиРассчитаны = Истина Тогда
        Для каждого СтрокаТабличнойЧасти Из Товары Цикл
            ПересчитатьСуммыВСтроке = Ложь;
            Если СтрокаТабличнойЧасти.СуммаСкидкиSailPlay > 0
                ИЛИ СтрокаТабличнойЧасти.СуммаСкидкиОплатыБонусомSailPlay > 0 Тогда
                ПересчитатьСуммыВСтроке = Истина;
            КонецЕсли; 
            СтрокаТабличнойЧасти.СуммаСкидкиSailPlay = 0;
            СтрокаТабличнойЧасти.ПроцентСкидкиНаценки = 0;
            СтрокаТабличнойЧасти.СуммаСкидкиОплатыБонусомSailPlay = 0;
            СтрокаТабличнойЧасти.ПримененнаяАкцияSailPlay = Неопределено;
            Если ПересчитатьСуммыВСтроке Тогда
                ОбработкаТабличныхЧастей.РассчитатьСуммуТабЧасти(СтрокаТабличнойЧасти,ЭтотОбъект);
                ОбработкаТабличныхЧастей.РассчитатьСуммуНДСТабЧасти(СтрокаТабличнойЧасти,ЭтотОбъект);
            КонецЕсли; 
        КонецЦикла; 
        SailPlay_СкидкиРассчитаны = Ложь;
    КонецЕсли; 
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
    ...
    //***{+Добавления}*** Тасмаджиев 06/08/2018// ->>
	//***{+Изменения}*** Тасмаджиев 09/08/2018// ->>
	//***=!{Закомментировал временно , в этой части ведется разработка ,для предупреждения ошибок в рабочей базе,}!=*** ->>
	СтруктураМакетов.Вставить("УниверсальныйПередаточныйДокумент981" , "Универсальный передаточный документ");
	//***{-Добавления}*** Тасмаджиев 06/08/2018// <<-
...
Функция РассчитыватьАвтоматическиеСкидки() Экспорт
	...
	// ins SailPlay 30.04.2016
    // подключена программа лояльности SailPlay
    // все акции и скидки делаются на стороне сервера
    Если ПараметрыСеанса.НеВыгружатьВSailPlay = Ложь Тогда
        Возврат Ложь;
    КонецЕсли; 
    //  end ins SailPlay 30.04.2016  
...
Процедура ЗаполнитьТабличнуюЧастьПоЗаказуПокупателяУпр(ТабличнаяЧасть, ДокументОснование) Экспорт
	...
	//pryan@inbox.ru 02/08/2015
	для н = 0 по Товары.Количество()-1 цикл
		если нрег(лев(сокрлп(Товары[н].Номенклатура.Наименование),5))="пакет" тогда
			Товары[н].Сумма = Товары[н].Цена;
			Товары[н].СуммаНДС = 0;
		конецесли;
	конеццикла;
	
	// Катков А. 15.06.2016 >> добавляем строки не заполненные стандартно, в соответствии с остатками
	Если ТаблицаПоЗаказу.Количество() > 0 Тогда
		
		ТабЗаказы = ТаблицаПоЗаказу.Скопировать(, "ЕдиницаИзмерения, КоличествоОстатокПоЗаказу, Коэффициент, КоэффициентЕдиницыХранения, Номенклатура, ПроцентСкидкиНаценки, СтавкаНДС, СуммаОстатокПоЗаказу, Цена, ПроцентАвтоматическихСкидок, УсловиеАвтоматическойСкидки, ЗначениеУсловияАвтоматическойСкидки");
		ТабЗаказы.Свернуть("ЕдиницаИзмерения, Коэффициент, КоэффициентЕдиницыХранения, Номенклатура, ПроцентСкидкиНаценки, СтавкаНДС, Цена, ПроцентАвтоматическихСкидок, УсловиеАвтоматическойСкидки, ЗначениеУсловияАвтоматическойСкидки", "КоличествоОстатокПоЗаказу, СуммаОстатокПоЗаказу");
		
		ТЧ_Товары = Товары.Выгрузить(, "Номенклатура, Количество, Цена, Сумма");//"Номенклатура, Количество, Цена, Сумма");
		ТЧ_Товары.Свернуть("Номенклатура, Цена", "Сумма, Количество");
		Для каждого СтрокаЗаказа из ТабЗаказы Цикл
			
			Если СтрокаЗаказа.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга Тогда
				Продолжить;
			КонецЕсли; 
			
			СтруктураПоиска = Новый Структура;
			СтруктураПоиска.Вставить("Номенклатура", СтрокаЗаказа.Номенклатура);
			СтруктураПоиска.Вставить("Цена", СтрокаЗаказа.Цена);
			
			МассивСтрокТЧ = ТЧ_Товары.НайтиСтроки(СтруктураПоиска);
			
			КоличествоВДоке = 0;
			СуммаВДоке = 0;
			Для каждого СтрокаМассива Из МассивСтрокТЧ Цикл
				КоличествоВДоке = КоличествоВДоке + СтрокаМассива.Количество;
				СуммаВДоке = СуммаВДоке + СтрокаМассива.Сумма;
			КонецЦикла;
			
			КоличествоЕдиницВЗаказе = ?(СтрокаЗаказа.Коэффициент = 0 , СтрокаЗаказа.КоличествоОстатокПоЗаказу, (СтрокаЗаказа.КоличествоОстатокПоЗаказу * СтрокаЗаказа.КоэффициентЕдиницыХранения / СтрокаЗаказа.Коэффициент));
			Если КоличествоВДоке < КоличествоЕдиницВЗаказе Тогда
				...
				СтрокаТабличнойЧасти.Количество       = КоличествоЕдиницВЗаказе - КоличествоВДоке;
				СтрокаТабличнойЧасти.ЕдиницаИзмерения = СтрокаЗаказа.ЕдиницаИзмерения;
				СтрокаТабличнойЧасти.Коэффициент      = СтрокаЗаказа.Коэффициент;
				...
				Сообщение.Текст = "Добавлена строка """ + СтрокаЗаказа.Номенклатура + """ " + (КоличествоЕдиницВЗаказе - КоличествоВДоке) + " ед. по цене " + СтрокаЗаказа.Цена + " (по позиции не было остатков)";
				Сообщение.Сообщить(); 
				// добавляем строку <<<<<<<
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "-------------------------------" + Символы.ПС + "Пожалуйста, сверьте сумму и сумму НДС с заказом.";
		Сообщение.Сообщить();
	...
	Если ОтражатьВУправленческомУчете Тогда
		...
		Если ВидПередачи = Перечисления.ВидыПередачиТоваров.СоСклада Тогда
			...
			Если Не РезультатЗапросаКомплектыОпт.Пустой() ИЛИ НЕ РезультатЗапросаТараОпт.Пустой() Тогда
				...
				// Катков А. 07.06.2016 <<  отключение проверки остатков на складе 
		...
		// Списываем из регистра товары на полке
		Если ЗначениеЗаполнено(ЭтотОбъект.Сделка) Тогда
			Если (ЭтотОбъект.Сделка.Дата>Дата(2014,06,19)) И (ЭтотОбъект.Склад = Справочники.Склады.НайтиПоКоду("000000005")) Тогда
				ТаблицаПоТоварамИзРезерва = ТаблицаПоТоварам.Скопировать();
				//Сч = 0;
				//Пока Сч < ТаблицаПоТоварамИзРезерва.Количество() Цикл
				//	СтрокаТаблицы = ТаблицаПоТоварамИзРезерва.Получить(Сч);
				...
				Если ТаблицаПоТоварамИзРезерва.Количество() > 0 Тогда
					...
					Если Не Отказ Тогда
						Движения.ТоварыНаПолке.ВыполнитьРасход();
					КонецЕсли;
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре) Экспорт
	...
	СтруктураПолей.Вставить("КоличествоСобрано"                  , "Количество * Коэффициент /Номенклатура.ЕдиницаХраненияОстатков.Коэффициент");
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПокупателя")
		...
		//проверяем оплату безнального заказа
		#Если Клиент Тогда
			Если (Основание.ВидОплаты <> Справочники.ВидыОплатЧекаККМ.Наличные) И (Основание.ВидОплаты <> Справочники.ВидыОплатЧекаККМ.ПустаяСсылка()) Тогда
				Если Основание.СтатусОплаты = Перечисления.СтатусОплаты.НеОпалчен Тогда
					//Предупреждение ("Заказ НЕ оплачен! Реализация невозможна!");
					Возврат;
				КонецЕсли;
			КонецЕсли;
		#КонецЕсли
		
		//++ Кузнецов С.А. - Задача в МегаПлане №3270 (Убираем ограничение) - 06.05.2016
		////если сумма заказа <10р и не подарочная карта, тогда уходим
		//#Если Клиент Тогда
		//	Если (Основание.СуммаДокумента<=10) И (Основание.ПодарочныеКарты.Количество()=0) Тогда
		//		Возврат;
		//	КонецЕсли;
		//#КонецЕсли	
		//-- Кузнецов С.А. - 06.05.2016		
		...
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
			Орг4Бух = Основание.Орг4Бух;
			//{+НАЧАЛО+Редактирования} Автор: Тасмаджиев 20/02/2018//==============================================================================================
			//{+НАЧАЛО+Добавления} Автор: Тасмаджиев 15/02/2018//==============================================================================================
			Если ЗначениеЗаполнено(Основание.Магазин) Тогда
				_5LB_Магазин = Основание.Магазин;
			КонецЕсли;
			Если  ЗначениеЗаполнено(Основание.Врозницу)Тогда
				_5LB_Врозницу = Основание.Врозницу;
			КонецЕсли;
			//{-КОНЕЦ-Добавления} Автор: Тасмаджиев 15/02/2018//==============================================================================================
			//{-КОНЕЦ-Редактирования} Автор: Тасмаджиев 20/02/2018//==============================================================================================
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка._5LB_ПлановоеПоступление") Тогда
		...
		Орг = _5LB_РаботаСНесколькимиЮрЛицами.ПолучитьКотрагентаПоОрганизации(Основание.Контрагент);
		...
		Контр = _5LB_РаботаСНесколькимиЮрЛицами.ПолучитьКотрагентаПоОрганизации(Основание.Организация);
		...
		ВидОперации   = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаКомиссия;
		ВидПередачи   = Перечисления.ВидыПередачиТоваров.СоСклада;
		Склад         = Основание.СкладРеализации;
		ЗаполнениеДокументов.ПриИзмененииЗначенияКонтрагента(ЭтотОбъект, мСтруктураПараметровДляПолученияДоговора);
		...
		Запрос.УстановитьПараметр("Склад",Основание.СкладРеализации);
	...
	//++ Кузнецов С.А. - Для изменения режима совместимости на 8.2.13 - 20.07.2016
	Если  ТипЗнч(Основание) = Тип("Структура") Тогда
		
	ИначеЕсли ТипЗнч(Основание) = Тип("Неопределено") Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//{+НАЧАЛО+Добавления} Автор: Тасмаджиев 03/07/2018//==============================================================================================
		Если ЭтоНовый() Тогда
		Если НЕ ЗначениеЗаполнено(ЭтотОбъект._5LB_СтатусПеревозки) Тогда
			ЭтотОбъект._5LB_СтатусПеревозки = Перечисления.СтатусПеревозки.Подготовлен;
		КонецЕсли;
	КонецЕсли;
	//{-КОНЕЦ-Добавления} Автор: Тасмаджиев 03/07/2018//==============================================================================================

	
	//07122016 Болкировка записи, если уже был пробит чек
	...
	//		             |	Р.Документ
	//		             |ИЗ
	//		             |	РегистрСведений.ЧекиККМ КАК Р
	//		             |ГДЕ
	//		             |	Р.Документ = &ЭтотОбъект";
	...
	ИтогСуммаД = УчетНДС.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "Товары") + УчетНДС.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "Услуги")-УчетНДС.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "ПодарочныеКарты");
	...
	Иначе
		...
		Для каждого СтрокаТаблицы Из ЭтотОбъект.Товары Цикл
			...
			//++ Кузнецов С.А. - Для исключение случаев при пробитии брака пустового качества - 22.08.2016
			Если СтрокаТаблицы.Качество = ПредопределенноеЗначение("Справочник.Качество.ПустаяСсылка") Тогда
				СтрокаТаблицы.Качество = ПредопределенноеЗначение("Справочник.Качество.Новый");
			КонецЕсли;	
			//-- Кузнецов С.А. - 22.08.2016
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//{+НАЧАЛО+Добавления} Автор: Тасмаджиев 04/06/2018//=================================================================================
	//================================================!{Первичная установка статуса перевозки}!===========================================
	Если Не Отказ Тогда
		Если НЕ ЗначениеЗаполнено(_5LB_СтатусПеревозки) Тогда
			//***{+Изменения}*** Тасмаджиев 18/07/2018// ->>
			_5LB_СтатусПеревозки = Перечисления.СтатусПеревозки.Подготовлен;
			//***{-Изменения}*** Тасмаджиев 18/07/2018// <<-
			КонецЕсли;
	КонецЕсли;	
	//{-КОНЕЦ-Добавления} Автор: Тасмаджиев 04/06/2018//===================================================================================
...
Функция ПересчитатьАвтоматическиеСкидки() Экспорт
	...
	//++ Кузнецов С.А. - Обнуляем скидки при изменении ТЧ "Товары" при использовании системы лояльности "SailPlay" - 30.06.2016
	Если ПараметрыСеанса.НеВыгружатьВSailPlay = Ложь Тогда
		SailPlay_СкидкиРассчитаны = Ложь;
		Для Каждого стрТовары Из Товары Цикл
			стрТовары.ПроцентАвтоматическихСкидок = 0;
			стрТовары.ПроцентСкидкиНаценки = 0;
			стрТовары.СуммаСкидкиSailPlay = 0;
            стрТовары.СуммаСкидкиОплатыБонусомSailPlay = 0;
            стрТовары.ПримененнаяАкцияSailPlay = Неопределено;
			ОбработкаТабличныхЧастей.РассчитатьСуммуТабЧасти(стрТовары, ЭтотОбъект);
			ОбработкаТабличныхЧастей.РассчитатьСуммуНДСТабЧасти(стрТовары, ЭтотОбъект);			
		КонецЦикла;	
	КонецЕсли;	
	...
	Иначе
		...
		// ins SailPlay 04.07.2016 
        ОбнулитьСкидкиSailPlay();
        //  end ins SailPlay 04.07.2016 
...
Функция МожноРассчитыватьАвтоматическиеСкидки() Экспорт
	...
	// ins SailPlay 30.04.2016 
    // все скидки идут только с сервера SailPlay
    Если ПараметрыСеанса.НеВыгружатьВSailPlay=Ложь Тогда
        Возврат Ложь;
    КонецЕсли; 
    //  end ins SailPlay 30.04.2016 

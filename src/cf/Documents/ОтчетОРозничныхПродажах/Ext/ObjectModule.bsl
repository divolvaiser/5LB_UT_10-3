...
Функция ПечатьОтчетаОПродажахККМ()
	...
	Запрос.УстановитьПараметр("Св", ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Адрес",Истина));
	...
	|	Док.Ссылка.Склад.Наименование КАК Склад,
	...
	|   Док.Ссылка.ZСуммаНач КАК СуммаНач,
	|   Док.Ссылка.ZСуммаКон КАК СуммаКон,
	|   Док.Ссылка.ZОтчетНомер КАК Z,
	|   Рег.Значение КАК Адрес
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.Товары КАК Док
	|   ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияСвойствОбъектов КАК Рег
	|   ПО Рег.Объект = Док.Ссылка.Склад
	|   И Рег.Свойство = &Св
	|ГДЕ
	...
	|	Док.Ссылка.Склад.Наименование,
	|   Док.Ссылка.ZСуммаНач,
	|   Док.Ссылка.ZСуммаКон,
	|   Док.Ссылка.ZОтчетНомер,
	|   Рег.Значение
	|";
	...
	ОбластьМакета.Параметры.ПодразделениеПредставление = Шапка.Склад+", "+Шапка.Адрес;
	...
	ОбластьМакета.Параметры.НомерДокумента           = Шапка.Z;//ОбщегоНазначения.ПолучитьНомерНаПечать(ЭтотОбъект);

...
Процедура ОбработкаЗаполнения(Основание)
	...
	// Было перенесено из ПеремещенияТоваров - ПередЗаписью , крашилась транзакция.
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПеремещениеТоваров")ИЛИ ТипЗнч(Основание) = Тип("ДокументОбъект.ПеремещениеТоваров") Тогда
		Если Основание.ВидОперации = Перечисления.ВидыОперацийПеремещениеТоваров.Транзит И Основание.Проведен=Ложь Тогда
			ЭтотОбъект.Дата = Основание.Дата;
			ЭтотОбъект.ВидЗаказа = Перечисления.ВидыВнутреннегоЗаказа.НаСклад;
			ЭтотОбъект.Заказчик = Основание.СкладПолучатель;
			ЭтотОбъект.Организация = Основание.Организация;
			ЭтотОбъект.Ответственный = Основание.Ответственный;
			ЭтотОбъект.Товары.Загрузить (Основание.Товары.Выгрузить(,"ЕдиницаИзмерения,ЕдиницаИзмеренияМест,Количество,КоличествоМест,Коэффициент,Номенклатура,ХарактеристикаНоменклатуры"));
			Для Каждого стр из ЭтотОбъект.Товары Цикл
				стр.Размещение = Основание.СкладОтправитель;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	//{-КОНЕЦ-Добавления} Автор: Тасмаджиев 09/06/2018//==============================================================================================

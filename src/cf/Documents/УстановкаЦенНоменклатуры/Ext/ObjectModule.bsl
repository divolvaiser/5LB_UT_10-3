...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ПриУстановкеСЦЗ_Артем_УстановитьТоварныйКурс();//09.11.2017, Юра
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЭтотОбъект.ТипыЦен.Количество() = 1 Тогда
		СЦЗ = Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("000000021");
		// 24.11.16 СредняяЦенаЗакупки 
		СредняяЦенаЗакупки = Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("000000015");
		Если (ЭтотОбъект.ТипыЦен[0].ТипЦен = СЦЗ ИЛИ ЭтотОбъект.ТипыЦен[0].ТипЦен = СредняяЦенаЗакупки)
			Тогда
			ЭтотОбъект.ЗаписыватьОперативно = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если  РольДоступна("_5LB_Установка_цен_номенклатуры_текущей_датой") Тогда
		ЭтотОбъект.Дата = ТекущаяДата();
	Иначе
		//{-Добавления} Автор: Тасмаджиев 13/07/2018// <<-
		
		Если НЕ ЭтотОбъект.ЗаписыватьОперативно	Тогда
			//	// если не "особый" и дата сегодня - установить дату на завтра 
			Если НачалоДня(ТекущаяДата()) = НачалоДня(ЭтотОбъект.Дата) Тогда
				ДтПоследнейЗаписи = ОбщегоНазначения5LB.ПолучитьВремяПоследнегоДокументаНаДату("УстановкаЦенНоменклатуры", КонецДня(ТекущаяДата()) + 1);
				// если до 1ч15 ночи - после
				ВремяПослеРегламента = 1*60*60 + 15*60;
				Если (ДтПоследнейЗаписи-НачалоДня(ДтПоследнейЗаписи)) < ВремяПослеРегламента Тогда 
					ДтПоследнейЗаписи = НачалоДня(ДтПоследнейЗаписи)+ ВремяПослеРегламента; 
					// иначе
				КонецЕсли;
				ЭтотОбъект.Дата = ДтПоследнейЗаписи + 1;
			КонецЕсли;			
		КонецЕсли;
		//{+Добавления} Автор: Тасмаджиев 13/07/2018// ->>
	КонецЕсли;
	...
	////КонецЕсли;
	//ДтПоследнейЗаписи = ОбщегоНазначения5LB.ПолучитьВремяПоследнегоДокументаНаДату("УстановкаЦенНоменклатуры", КонецДня(ТекущаяДата());
	//ЭтотОбъект.Дата = ДтПоследнейЗаписи + 1;
...
//09.11.2017, Юра
Процедура ПриУстановкеСЦЗ_Артем_УстановитьТоварныйКурс()
	Если НЕ ТипыЦен.Найти(Справочники.ТипыЦенНоменклатуры.СЦЗ_Артем,"ТипЦен") = Неопределено Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               |	УстановкаЦенНоменклатурыТовары.Номенклатура
		               |ПОМЕСТИТЬ ВсяНоменклатураИзДокумента
		               |ИЗ
		               ...
		               |	КурсоваяНаценкаСрезПоследних.ТоварныйКурс КАК ТекущийТоварныйКурс,
		               |	ЕСТЬNULL(КурсыВалютСрезПоследних.Курс, 0) * ЕСТЬNULL(КурсыВалютСрезПоследних.Кратность, 0) КАК ТекущийКурсВалюты
		               |ИЗ
		               |	РегистрСведений.КурсоваяНаценка.СрезПоследних(
		               |			&ДатаДокумента,
		               |			Номенклатура В
		               |				(ВЫБРАТЬ
		               |					ВсяНоменклатураИзДокумента.Номенклатура
		               |				ИЗ
		               |					ВсяНоменклатураИзДокумента КАК ВсяНоменклатураИзДокумента)) КАК КурсоваяНаценкаСрезПоследних
		               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&ДатаДокумента, ) КАК КурсыВалютСрезПоследних
		               |		ПО КурсоваяНаценкаСрезПоследних.Валюта = КурсыВалютСрезПоследних.Валюта
		               |ГДЕ
		               |	КурсоваяНаценкаСрезПоследних.ТоварныйКурс <> ЕСТЬNULL(КурсыВалютСрезПоследних.Курс, 0) * ЕСТЬNULL(КурсыВалютСрезПоследних.Кратность, 0)";
		Запрос.УстановитьПараметр("ДатаДокумента",Дата);
		Запрос.УстановитьПараметр("Ссылка",Ссылка);
		РезультатЗапроса = Запрос.Выполнить();
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Мен = РегистрыСведений.КурсоваяНаценка.СоздатьМенеджерЗаписи();
			Мен.Период = Дата;
			Мен.Номенклатура = ВыборкаДетальныеЗаписи.Номенклатура; 
			Мен.ТоварныйКурс = ВыборкаДетальныеЗаписи.ТекущийКурсВалюты;
			Мен.Валюта = ВыборкаДетальныеЗаписи.Валюта;
			Мен.Записать();
		КонецЦикла;

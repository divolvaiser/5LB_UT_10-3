...
Функция ОпределитьСтатусПартииПриходаУпр(СтруктураШапкиДокумента)
	...
	СтатусПартииПриходаУпр = Перечисления.СтатусыПартийТоваров.ПустаяСсылка(); 	//НМА 11.10.17

	Если СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомитентом Тогда
		СтатусПартииПриходаУпр = Перечисления.СтатусыПартийТоваров.НаКомиссию;  //НМА 11.10.17
	Иначе
		СтатусПартииПриходаУпр = Перечисления.СтатусыПартийТоваров.Купленный;   //НМА 11.10.17
	КонецЕсли;
		
	Возврат СтатусПартииПриходаУпр; //НМА 11.10.17
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		...
		//{+НАЧАЛО+Добавления} Автор: Тасмаджиев 21/02/2018//==============================================================================================
		
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка._5LB_ЗаявкаНаВозвратТоваровПоставщику") Тогда
		// Заполнение шапки
		ОтражатьВУправленческомУчете = Истина;
		ВалютаДокумента              = Основание.ВалютаДокумента;
		ДоговорКонтрагента           = Основание.ДоговорКонтрагента;
		...
		Для Каждого ТекСтрокаТовары Из Основание.Товары Цикл
			...
			НоваяСтрока.Цена                       = ТекСтрокаТовары.Цена;
		КонецЦикла;
		//{-КОНЕЦ-Добавления} Автор: Тасмаджиев 21/02/2018//==============================================================================================
		//***{+Добавления}*** Тасмаджиев 02/08/2018// ->>
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки_УТ11") Тогда
		СтруктураПоиска = Новый Структура();
		СтруктураПоиска.Вставить("Решение",Перечисления._5LB_РешенияПоПретензионнымТоварам.ВозвратПоставщику);
		...
		|	Документ.ВозвратТоваровПоставщику КАК ВозвратТоваровПоставщику
		|ГДЕ
		|	ВозвратТоваровПоставщику.ПометкаУдаления = ЛОЖЬ
		|	И ВозвратТоваровПоставщику.Сделка = &Сделка";
		...
		Если Основание.Товары.НайтиСтроки(СтруктураПоиска).Количество()<> 0 Тогда
			...
			Если Результат.Пустой() Тогда //Если поступление не создавалось , то создаем списание и поступление.
				ЗаполнениеДокументов.ЗаполнитьШапкуДокументаПоОснованию(ЭтотОбъект, Основание);
				ЭтотОбъект.Склад = ПолучитьПретензионныйСклад(Основание.СкладОрдер);
				ЭтотОбъект.Дата = ТекущаяДата();
				ЭтотОбъект.Сделка = Основание.Ссылка;
				ЗаполнитьТоварыПоАктуОрасхождениях(Основание, Ложь);
				
			Иначе 
				//Заполняем Инвентаризацию для того чтобы при открытии 
	...
	Если не мСтруктураПараметровВзаиморасчетов.Свойство("Дата") тогда 
		мСтруктураПараметровВзаиморасчетов.Вставить("Дата",ТекущаяДата());
	Иначе
...
Процедура ЗаполнитьТоварыПоАктуОрасхождениях(ДокументОснование, ПодтверждатьОчисткуСтрок) Экспорт
	...
	Для каждого Строка ИЗ ДокументОснование.Товары Цикл 
		...
		Если Строка.Решение = Перечисления._5LB_РешенияПоПретензионнымТоварам.ВозвратПоставщику Тогда
...
КонецПроцедуры //***{-Добавления}*** Тасмаджиев 02/08/2018// <<-
...
//***{+Добавления}*** Тасмаджиев 02/08/2018// ->>
Функция НайтиНеНайденныйСебест (Ном)
	Себест=0;
	Отбор = Новый Структура ("Номенклатура");
	Отбор.Номенклатура = Ном;
	Выб = РегистрыНакопления.ПартииТоваровНаСкладах.Выбрать(,,Отбор,"Убыв");
	Пока Выб.Следующий() Цикл
		Если (Выб.Количество>0) и (Выб.Склад = ЭтотОбъект.Склад) Тогда
			Себест = ОКР(Выб.Стоимость/Выб.Количество,2,1);
			Если Себест>0 Тогда
				прервать;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Если Себест<=0 Тогда
		Сообщить ("Проблемы с ценой у товара "+Ном+"!",СтатусСообщения.Важное);
	КонецЕсли;
	Возврат Себест;
КонецФункции //***{-Добавления}*** Тасмаджиев 02/08/2018// <<-

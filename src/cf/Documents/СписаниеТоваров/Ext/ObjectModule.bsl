...
Процедура ЗаполнитьТоварыПоИнвентаризацииТоваровУпр(ДокументОснование, ПодтверждатьОчисткуСтрок) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		//Если ПолучитьТекущийВидСклада() = Перечисления.ВидыСкладов.НТТ Тогда
			...
			СтрокаТабличнойЧасти.Цена = ?(Выборка.Цена>0,Выборка.Цена,НайтиНеНайденныйСебест (Выборка.Номенклатура));
...
Процедура ЗаполнитьТоварыПоАктуОрасхождениях(ДокументОснование, ПодтверждатьОчисткуСтрок) Экспорт
	...
	Для каждого Строка ИЗ ДокументОснование.Товары Цикл 
		...
		Если Строка.Решение = Перечисления._5LB_РешенияПоПретензионнымТоварам.Списать Тогда
...
//pryan@inbox.ru 10/12/2013
Процедура ЗаполнитьЦеныПоТипу(Данные) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ПО тзНоменклатура.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура";
	...
	для каждого стр из Товары цикл
		Строка = тзРезультат.Найти(стр.Номенклатура,"Номенклатура");
		если не Строка = неопределено тогда
			Если стр.ЕдиницаИзмерения<>Строка.ЕдиницаИзмерения Тогда
				 Попытка
					стр.Цена = ОКР(Строка.Цена/Строка.ЕдиницаИзмерения.Коэффициент,2,1);
				Исключение
...
Процедура ОбработкаЗаполнения(Основание)
	...
	//***{+Добавления}*** Тасмаджиев 02/08/2018// ->>
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки_УТ11") Тогда
		СтруктураПоиска = Новый Структура();
		СтруктураПоиска.Вставить("Решение",Перечисления._5LB_РешенияПоПретензионнымТоварам.Списать);
		...
		|	И СписаниеТоваров.ИнвентаризацияТоваровНаСкладе = &Сделка";
		...
		Если Основание.Товары.НайтиСтроки(СтруктураПоиска).Количество()<> 0 Тогда
			...
			Если Результат.Пустой() Тогда //Если поступление не создавалось , то создаем списание и поступление.
				...
				ЭтотОбъект.ИнвентаризацияТоваровНаСкладе = Основание.Ссылка;
			...
			Иначе 
				...
				//(ТипЗнч(Инвентаризацию.Ссылка) = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки_УТ11"))

...
Функция ПечатьВозвратаОтПокупателя()
	...
	|	ЕСТЬNULL(Рег.СтоимостьДоставки, 0) КАК СтоимостьДоставки
	|ИЗ
	|	Документ.ВозвратТоваровОтПокупателя КАК ВозвратТоваровОтПокупателя
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВозвратыДопДанные КАК Рег
	|		ПО (Рег.Заказ = ВозвратТоваровОтПокупателя.Ссылка)
	|ГДЕ
	...
	Штрих = ИщемШтрихВозврата();
	Если Штрих = Неопределено Тогда
		Штрих = СоздаёмШтрихВозврата();
	...
	ИначеЕсли Штрих = "Косяк" Тогда
		Предупреждение ("Нарушение штрих-кодирования возвратов! Обратитесь к руководству!");
	Иначе
		ОбластьМакета.Рисунки.Штрихкод.Объект.ТипКода = 1;
		ОбластьМакета.Рисунки.Штрихкод.Объект.Сообщение = Штрих;
	КонецЕсли;
	...
	Пока ВыборкаСтрокТовары.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаСтрокТовары.ДокументПартии) Тогда								
		ОбластьСуммы.Параметры.Сумма = -ОбластьСуммы.Параметры.Сумма;
		КонецЕсли;

		ТабДокумент.Присоединить(ОбластьСуммы);
		Сумма          = Сумма       + ?(ЗначениеЗаполнено(ВыборкаСтрокТовары.ДокументПартии),ВыборкаСтрокТовары.Сумма,0);
	...
	Если ЕстьСкидки Тогда
		...
		ТабДокумент.Присоединить(ОбластьСкидок);
	КонецЕсли;
	ОбластьСуммы.Параметры.СтоимостьДоставки = -Шапка.СтоимостьДоставки;
	ОбластьСуммы.Параметры.Всего = ОбщегоНазначения.ФорматСумм(Сумма-Шапка.СтоимостьДоставки);
	ТабДокумент.Присоединить(ОбластьСуммы);
	...
	СуммаКПрописи                          = Сумма + ?(Шапка.СуммаВключаетНДС, 0, СуммаНДС)-Шапка.СтоимостьДоставки;
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "Накладная" Тогда
		Если РольДоступна(Метаданные.Роли.КладOFFщик) Тогда
			Если (ВидОперации = Перечисления.ВидыОперацийВозвратТоваровОтПокупателя.ВозвратОбмен) и (СтатусСборки=Перечисления.СтатусыСборкиЗаказа.Собран) Тогда
				СтатусСборки=Перечисления.СтатусыСборкиЗаказа.УКурьера;
				Попытка
					ЭтотОбъект.Записать(РежимЗаписиДокумента.Запись);
					// Получить экземпляр документа на печать
					ТабДокумент = ПечатьВозвратаОтПокупателя();
				Исключение
					Сообщить ("Не удалось распечатать документ! Повторите попытку");
				КонецПопытки;
			КонецЕсли;
		Иначе
			ТабДокумент = ПечатьВозвратаОтПокупателя();
...
Процедура ЗаполнитьТоварыПоОснованию(ДокументОснование) Экспорт
	...
	|")
    // ins SailPlay 28.04.2016 
    + ?(ОснованиеРеализацияТоваров,", Док.НомерСтроки КАК КлючСвязиSailPlay","")
    //  end ins SailPlay 28.04.2016 
    ;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОснованиеРеализацияТоваров Тогда
            ...
            // ins SailPlay 28.04.2016 
            Попытка
                СтрокаТабличнойЧасти.КлючСвязиSailPlay  = Выборка.КлючСвязиSailPlay;
            Исключение
            КонецПопытки; 
            //  end ins SailPlay 28.04.2016 
		КонецЕсли;
...
Процедура ЗаполнитьТоварыНереализованнымиУпр(СпособЗаполнения, ДокументРеализации = Неопределено) Экспорт
	...
	Если СпособЗаполнения = "ПоДокументу" Тогда
		...
		|		1
		|	КОНЕЦ                                                    КАК КратностьДокумента,
		|   Док.НомерСтроки                                          КАК КлючСвязиSailPlay, // ins SailPlay 28.04.2016 
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		Сделка         = ?(ЗначениеЗаполнено(Основание.Сделка),Основание.Сделка,Основание); // заполняем сделку заказом покупателя или счетом на оплату
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПриходныйОрдерНаТовары") Тогда
		...
		ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка._5LB_ПлановаяОтгрузка") Тогда
		
		// Заполним реквизиты шапки по документу основанию.
		ЗаполнениеДокументов.ЗаполнитьШапкуДокументаПоОснованию(ЭтотОбъект, Основание);
		ВидОперации    = Перечисления.ВидыОперацийВозвратТоваровОтПокупателя.ПростойВозврат;
		ВидПоступления = Перечисления.ВидыПоступленияТоваров.НаСклад;
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	_5LB_ПлановаяОтгрузкаЗаказыПокупателей.Заказ
		|ПОМЕСТИТЬ ВТ_Заказы
		|ИЗ
		|	Документ._5LB_ПлановаяОтгрузка.ЗаказыПокупателей КАК _5LB_ПлановаяОтгрузкаЗаказыПокупателей
		|ГДЕ
		|	_5LB_ПлановаяОтгрузкаЗаказыПокупателей.Ссылка = &Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидОперации=Перечисления.ВидыОперацийВозвратТоваровОтПокупателя.ВозвратОбмен Тогда
		Сумм=0;
		Отбор = Новый Структура ("ДокументПартии",Неопределено);
		Строки = Товары.НайтиСтроки(Отбор);
		Для каждого строка из Строки Цикл
			Сумм = Сумм+строка.Сумма;
		КонецЦикла;
		СуммаДокумента = СуммаДокумента-Сумм;
	КонецЕсли;
...
Функция ИщемШтрихВозврата()  Экспорт
	Штрих = Неопределено;
...
Функция СоздаёмШтрихВозврата() Экспорт
	...
	ТекЗапись.Штрихкод         = РаботаСТорговымОборудованием.СформироватьШтрихКодВозврата();

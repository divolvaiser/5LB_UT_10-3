...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Если НЕ ПустаяСтрока(ИмяРегистра) Тогда 
		...
		Если ИмяРегистра = "ТоварыВРезервеНаСкладах" Тогда
			...
			|	ТаблицаРегистра.ДокументРезерва.* КАК ДокументРезерва,
			|	ТаблицаРегистра.СерияНоменклатуры.* КАК СерияНоменклатуры,
			|	Перемещение.Ссылка.* КАК Перемещение,
			|	(СУММА(ТаблицаРегистра.КоличествоОстаток)) КАК Количество}
			|ИЗ
			|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки(&ДатаКон, {(Склад).* КАК Склад, (Номенклатура).* КАК Номенклатура, (ХарактеристикаНоменклатуры).* КАК ХарактеристикаНоменклатуры, (ДокументРезерва).* КАК ДокументРезерва, (СерияНоменклатуры).* КАК СерияНоменклатуры}) КАК ТаблицаРегистра
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПеремещениеТоваров КАК Перемещение
			|		ПО (Перемещение.ВнутреннийЗаказ = ТаблицаРегистра.ДокументРезерва)
			|{ГДЕ
			|	ТаблицаРегистра.Склад.* КАК Склад,
			|	ТаблицаРегистра.Номенклатура.* КАК Номенклатура,
			|	ТаблицаРегистра.ХарактеристикаНоменклатуры.* КАК ХарактеристикаНоменклатуры,
...
Процедура Настроить(СтруктураПараметров, ЗаполняемыйОбъект = Неопределено) Экспорт
	...
	ВыводитьДополнительныеПолявОтдельнойКолонке = Истина; //Параметры["ВыводитьДополнительныеПолявОтдельнойКолонке"];
		...
		Если ВыводитьДополнительныеПоляВОтдельнойКолонке Тогда
			...
			Для Сч = 0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1 Цикл
				...
				Для Инд = 0 По ПостроительОтчета.ВыбранныеПоля.Количество()-1 Цикл
					...
					//ВнешнийМакетТоварыВРезерве->
					если Измерение.ПутьКДанным = "Перемещение" И ПостроительОтчета.ВыбранныеПоля[Инд].ПутьКДанным = "Перемещение.СтатусСборки" тогда
						продолжить
					конецесли;	
					//<-ВнешнийМакетТоварыВРезерве
...
//ВнешнийМакетТоварыВРезерве->
	ВнешнийМакетТоварыВРезерве = Ложь;
	если ПостроительОтчета.ИзмеренияСтроки.Найти("ДокументРезерва") <> неопределено И  ПостроительОтчета.ИзмеренияСтроки.Найти("Перемещение") <> неопределено тогда
		если ПостроительОтчета.ВыбранныеПоля.Найти("ДокументРезерваСтатусыСборкиЗаказа") <> неопределено И ПостроительОтчета.ВыбранныеПоля.Найти("ПеремещениеСтатусСборки") <> неопределено тогда
			ВнешнийМакетТоварыВРезерве = Истина;
		конецесли;
	конецесли;	
//<-ВнешнийМакетТоварыВРезерве
	...
	Если ПостроительОтчета.ИзмеренияКолонки.Количество() = 0 Тогда
//ВнешнийМакетТоварыВРезерве->
если ВнешнийМакетТоварыВРезерве тогда
	...
	ДокументРезультат.Вывести(ПостроительОтчета.МакетШапкиТаблицы);
	
	ОбластьСтрокаДокументРезерва = ПолучитьМакет("МакетТоварыВРезерве").ПолучитьОбласть("СтрокаДокументРезерва");
	ОбластьСтрокаПеремещение = ПолучитьМакет("МакетТоварыВРезерве").ПолучитьОбласть("СтрокаПеремещение");
	
	ДокументРезультат.НачатьАвтогруппировкуСтрок();
	
	ОбходОбщиеИтоги = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.Прямой);
	Пока ОбходОбщиеИтоги.Следующий() Цикл
		//28.02.2017 Запрос от склада: не видны заказы в таблице
		Если ОбходОбщиеИтоги.Уровень()<>3 Тогда
			Продолжить;
		КонецЕсли;
		ОбластьСтрокаДокументРезерва.Параметры.Документ = ?(ЗначениеЗаполнено(ОбходОбщиеИтоги.Перемещение),0,ОбходОбщиеИтоги.ДокументРезерва);
		ОбластьСтрокаДокументРезерва.Параметры.Статус = ?(ЗначениеЗаполнено(ОбходОбщиеИтоги.Перемещение),0,ОбходОбщиеИтоги.ДокументРезерваСтатусыСборкиЗаказа);
		ОбластьСтрокаДокументРезерва.Параметры.Количество = ОбходОбщиеИтоги.Количество;
		ДокументРезультат.Вывести(ОбластьСтрокаДокументРезерва,0);
		
		ОбластьСтрокаПеремещение.Параметры.Документ = ОбходОбщиеИтоги.Перемещение;
		ОбластьСтрокаПеремещение.Параметры.Статус = ОбходОбщиеИтоги.ПеремещениеСтатусСборки;
		ОбластьСтрокаПеремещение.Параметры.Количество = ОбходОбщиеИтоги.Количество;
		ДокументРезультат.Вывести(ОбластьСтрокаПеремещение,1);			
		
		
		
		
		//ОбходДокументРезерва = ОбходОбщиеИтоги.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		//Пока ОбходДокументРезерва.Следующий() Цикл
		//	ОбластьСтрокаДокументРезерва.Параметры.Документ = ОбходДокументРезерва.ДокументРезерва;
		//	ОбластьСтрокаДокументРезерва.Параметры.Статус = ОбходДокументРезерва.ДокументРезерваСтатусыСборкиЗаказа;
		//	ОбластьСтрокаДокументРезерва.Параметры.Количество = ОбходДокументРезерва.Количество;
		//	ДокументРезультат.Вывести(ОбластьСтрокаДокументРезерва,0);			
		//	
		//	ОбходПеремещение = ОбходДокументРезерва.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		//	Пока ОбходПеремещение.Следующий() Цикл
		//		ОбластьСтрокаПеремещение.Параметры.Документ = ОбходПеремещение.Перемещение;
		//		ОбластьСтрокаПеремещение.Параметры.Статус = ОбходПеремещение.ПеремещениеСтатусСборки;
		//		ОбластьСтрокаПеремещение.Параметры.Количество = ОбходПеремещение.Количество;
		//		ДокументРезультат.Вывести(ОбластьСтрокаПеремещение,1);			
		//	Конеццикла;	
		//Конеццикла;	
		
		//// 4. Вывод итогов
		//ПостроительОтчета.МакетОбщихИтогов.Параметры.Количество = ОбходОбщиеИтоги.Количество;
		//ДокументРезультат.Вывести(ПостроительОтчета.МакетОбщихИтогов,0);
	конеццикла;	
	// 4. Вывод итогов
	ПостроительОтчета.МакетОбщихИтогов.Параметры.Количество = ОбходОбщиеИтоги.Количество;
	ДокументРезультат.Вывести(ПостроительОтчета.МакетОбщихИтогов,0);

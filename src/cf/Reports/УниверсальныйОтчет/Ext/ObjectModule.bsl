...
//pryan@inbox.ru 15/02/2014
Перем тзРезультатБонусыРасшифровка;

Процедура РасшифровкаБонусов(Расшифровка, ПараметрБонусыПродавцам)
	
	если ТипЗнч(тзРезультатБонусыРасшифровка) <> Тип("ТаблицаЗначений") тогда
		возврат;
	конецесли;
	
	если тзРезультатБонусыРасшифровка.Количество() = 0 тогда
		возврат;
	конецесли;
	
	НоменклатураРодитель = Расшифровка.НоменклатураРодитель;
	ДокументПродажиОтветственный = Расшифровка.ДокументПродажиОтветственный;
	
	Данные = неопределено;
	СменДней = 0;
	если сокрлп(НоменклатураРодитель) <> "" и сокрлп(ДокументПродажиОтветственный) <> "" тогда
		Отбор = новый Структура;
		Отбор.Вставить("НоменклатураРодитель",НоменклатураРодитель);
		Отбор.Вставить("ДокументПродажиОтветственный",ДокументПродажиОтветственный);
		
		Данные = тзРезультатБонусыРасшифровка.НайтиСтроки(Отбор);
		если Данные.Количество() = 0 тогда
			Данные = неопределено;
		иначе
			тзДанные = тзРезультатБонусыРасшифровка.Скопировать(Данные);
			тзСмен = тзДанные.Скопировать();
			тзСмен.Свернуть("ДокументПродажиОтветственный,ДокументПродажиДата");
			СменДней = тзСмен.Количество();
			если СменДней > 0 тогда
				для н = 0 по тзСмен.Количество() - 1 цикл
					Отбор = новый Структура;
					Отбор.Вставить("ДокументПродажиОтветственный",тзСмен[н].ДокументПродажиОтветственный);
					Отбор.Вставить("ДокументПродажиДата",тзСмен[н].ДокументПродажиДата);
					СтрокиДанных = тзДанные.НайтиСтроки(Отбор); 
					если СтрокиДанных.Количество() > 0 тогда
						СтрокиДанных[0].СменДней = 1;
					конецесли;
				конеццикла;
			конецесли;	
		конецесли;
	иначеесли сокрлп(НоменклатураРодитель) <> "" и сокрлп(ДокументПродажиОтветственный) = "" тогда	
		Отбор = новый Структура;
		Отбор.Вставить("НоменклатураРодитель",НоменклатураРодитель);
	...
	иначе //всё
		Данные = 0;
		тзДанные = тзРезультатБонусыРасшифровка; 	
	...
	Макет = ПараметрБонусыПродавцам.Макет;
	ОбластьШ = Макет.ПолучитьОбласть("ш");
	ОбластьСтр = Макет.ПолучитьОбласть("стр");
	ОбластьИтог = Макет.ПолучитьОбласть("итог");
	
	Таблица.Вывести(ОбластьШ);
	...
	ВычетСПродавцаРуб = 0;
	БонусРубУц = 0;
	для н = 0 по тзДанные.Количество()-1 цикл
		ОбластьСтр.Параметры.Заполнить(тзДанные[н]);
		Таблица.Вывести(ОбластьСтр);
		
		СтоимостьОборот = СтоимостьОборот + тзДанные[н].СтоимостьОборот;
		БонусРуб = БонусРуб + тзДанные[н].БонусРуб;
		//СменДней = СменДней + тзДанные[н].СменДней;
		если ПараметрБонусыПродавцам.ВыводитьУценку тогда
			БонусРубОсн = БонусРубОсн + тзДанные[н].БонусРубОсн;
			БонусПродавцуРуб = БонусПродавцуРуб + тзДанные[н].БонусПродавцуРуб;
			ВычетСПродавцаРуб = ВычетСПродавцаРуб + тзДанные[н].ВычетСПродавцаРуб;
			БонусРубУц = БонусРубУц + тзДанные[н].БонусРубУц;
		конецесли;	
	конеццикла;	
	
	ОбластьИтог.Параметры.СтоимостьОборот = СтоимостьОборот;
	ОбластьИтог.Параметры.БонусРуб = БонусРуб;
	ОбластьИтог.Параметры.СменДней = СменДней;
	если ПараметрБонусыПродавцам.ВыводитьУценку тогда
		ОбластьИтог.Параметры.БонусРубОсн = БонусРубОсн;
		ОбластьИтог.Параметры.БонусПродавцуРуб = БонусПродавцуРуб;
		ОбластьИтог.Параметры.ВычетСПродавцаРуб = ВычетСПродавцаРуб;
		ОбластьИтог.Параметры.БонусРубУц = БонусРубУц;
	конецесли;	
	Таблица.Вывести(ОбластьИтог);
	
	Таблица.Показать("Расшифровка Бонусы продавцам"+?(сокрлп(НоменклатураРодитель)=""," "," "+сокрлп(НоменклатураРодитель))+?(сокрлп(ДокументПродажиОтветственный)="",""," "+сокрлп(ДокументПродажиОтветственный)));
КонецПроцедуры
	
Процедура СформироватьОтчетБонусы(ТабличныйДокумент,БонусыМакет,ПараметрБонусыПродавцам,тзРезультатБонусы)
	
	тзРезультатБонусыРасшифровка = тзРезультатБонусы.Скопировать();
	
	состояние("Вывод данных по продавцам");
	
	тзСмены = тзРезультатБонусы.Скопировать();
	тзСмены.Свернуть("ДокументПродажиОтветственный,ДокументПродажиДата");
	                                                                             
	тзПродавцы = тзРезультатБонусы.Скопировать();
	если ПараметрБонусыПродавцам.ВыводитьУценку тогда
		тзПродавцы.Свернуть("ДокументПродажиОтветственный","СтоимостьОборот,БонусРубОсн,БонусРубУц,БонусРуб,СменДней");
	иначе	
		тзПродавцы.Свернуть("ДокументПродажиОтветственный","СтоимостьОборот,БонусРуб,СменДней");
	конецесли;
	
	тзГруппы = тзРезультатБонусы.Скопировать();
	тзГруппы.Свернуть("НоменклатураРодитель","СтоимостьОборот,БонусРуб");
	
	тзГруппыПродавцы = тзРезультатБонусы.Скопировать();
	если ПараметрБонусыПродавцам.ВыводитьУценку тогда
		тзГруппыПродавцы.Свернуть("НоменклатураРодитель,ДокументПродажиОтветственный,БонусПроцент","СтоимостьОборот,БонусРубОсн,БонусРубУц,БонусРуб,СменДней");
	иначе	
		тзГруппыПродавцы.Свернуть("НоменклатураРодитель,ДокументПродажиОтветственный,БонусПроцент","СтоимостьОборот,БонусРуб,СменДней");
	конецесли;
	...
	ОбластьШ_Осн = БонусыМакет.ПолучитьОбласть("ш|осн");
	ОбластьШ_Продавец_Чет = БонусыМакет.ПолучитьОбласть("ш|продавец");
	ОбластьШ_БонусыПроцент_Чет = БонусыМакет.ПолучитьОбласть("ш|бонусыПроцент");
	ОбластьШ_БонусыРубОсн_Чет = БонусыМакет.ПолучитьОбласть("ш|бонусыРубОсн");
	ОбластьШ_БонусыРубУц_Чет = БонусыМакет.ПолучитьОбласть("ш|бонусыРубУц");
	ОбластьШ_БонусыРуб_Чет = БонусыМакет.ПолучитьОбласть("ш|бонусыРуб");
	ОбластьШ_Смены_Чет = БонусыМакет.ПолучитьОбласть("ш|смены");
	
	ОбластьШ_Всего = БонусыМакет.ПолучитьОбласть("ш|всего");
	ОбластьШ_ВсегоБонусы = БонусыМакет.ПолучитьОбласть("ш|всегоБонусы");
	
	ОбластьШ_Продавец_Нечет = БонусыМакет.ПолучитьОбласть("ш|продавец1");
	ОбластьШ_БонусыПроцент_Нечет = БонусыМакет.ПолучитьОбласть("ш|бонусыПроцент1");
	ОбластьШ_БонусыРубОсн_Нечет = БонусыМакет.ПолучитьОбласть("ш|бонусыРубОсн1");
	ОбластьШ_БонусыРубУц_Нечет = БонусыМакет.ПолучитьОбласть("ш|бонусыРубУц1");
	ОбластьШ_БонусыРуб_Нечет = БонусыМакет.ПолучитьОбласть("ш|бонусыРуб1");
	ОбластьШ_Смены_Нечет = БонусыМакет.ПолучитьОбласть("ш|смены1");
	
	ОбластьСтрока_Осн = БонусыМакет.ПолучитьОбласть("строка|осн");
	ОбластьСтрока_Продавец_Чет = БонусыМакет.ПолучитьОбласть("строка|продавец");
	ОбластьСтрока_БонусыПроцент_Чет = БонусыМакет.ПолучитьОбласть("строка|бонусыПроцент");
	ОбластьСтрока_БонусыРубОсн_Чет = БонусыМакет.ПолучитьОбласть("строка|бонусыРубОсн");
	ОбластьСтрока_БонусыРубУц_Чет = БонусыМакет.ПолучитьОбласть("строка|бонусыРубУц");
	ОбластьСтрока_БонусыРуб_Чет = БонусыМакет.ПолучитьОбласть("строка|бонусыРуб");
	ОбластьСтрока_Смены_Чет = БонусыМакет.ПолучитьОбласть("строка|смены");
	
	ОбластьСтрока_Всего = БонусыМакет.ПолучитьОбласть("строка|всего");
	ОбластьСтрока_ВсегоБонусы = БонусыМакет.ПолучитьОбласть("строка|всегоБонусы");
	
	ОбластьСтрока_Продавец_Нечет = БонусыМакет.ПолучитьОбласть("строка|продавец1");
	ОбластьСтрока_БонусыПроцент_Нечет = БонусыМакет.ПолучитьОбласть("строка|бонусыПроцент1");
	ОбластьСтрока_БонусыРубОсн_Нечет = БонусыМакет.ПолучитьОбласть("строка|бонусыРубОсн1");
	ОбластьСтрока_БонусыРубУц_Нечет = БонусыМакет.ПолучитьОбласть("строка|бонусыРубУц1");
	ОбластьСтрока_БонусыРуб_Нечет = БонусыМакет.ПолучитьОбласть("строка|бонусыРуб1");
	ОбластьСтрока_Смены_Нечет = БонусыМакет.ПолучитьОбласть("строка|смены1");
	
	ОбластьИтог_Осн = БонусыМакет.ПолучитьОбласть("итог|осн");
	ОбластьИтог_Продавец_Чет = БонусыМакет.ПолучитьОбласть("итог|продавец");
	ОбластьИтог_БонусыПроцент_Чет = БонусыМакет.ПолучитьОбласть("итог|бонусыПроцент");
	ОбластьИтог_БонусыРубОсн_Чет = БонусыМакет.ПолучитьОбласть("итог|бонусыРубОсн");
	ОбластьИтог_БонусыРубУц_Чет = БонусыМакет.ПолучитьОбласть("итог|бонусыРубУц");
	ОбластьИтог_БонусыРуб_Чет = БонусыМакет.ПолучитьОбласть("итог|бонусыРуб");
	ОбластьИтог_Смены_Чет = БонусыМакет.ПолучитьОбласть("итог|смены");
	
	ОбластьИтог_Всего = БонусыМакет.ПолучитьОбласть("итог|всего");
	ОбластьИтог_ВсегоБонусы = БонусыМакет.ПолучитьОбласть("итог|всегоБонусы");
	
	ОбластьИтог_Продавец_Нечет = БонусыМакет.ПолучитьОбласть("итог|продавец1");
	ОбластьИтог_БонусыПроцент_Нечет = БонусыМакет.ПолучитьОбласть("итог|бонусыПроцент1");
	ОбластьИтог_БонусыРубОсн_Нечет = БонусыМакет.ПолучитьОбласть("итог|бонусыРубОсн1");
	ОбластьИтог_БонусыРубУц_Нечет = БонусыМакет.ПолучитьОбласть("итог|бонусыРубУц1");
	ОбластьИтог_БонусыРуб_Нечет = БонусыМакет.ПолучитьОбласть("итог|бонусыРуб1");
	ОбластьИтог_Смены_Нечет = БонусыМакет.ПолучитьОбласть("итог|смены1");
	
	Отбор = новый Структура;
	
	ТабличныйДокумент.НачатьАвтогруппировкуКолонок();
	
	ТабличныйДокумент.Вывести(ОбластьШ_Осн,0);
	для н = 0 по тзПродавцы.Количество()-1 цикл
		если окр(н / 2,0,1) <> н / 2 тогда
			ОбластьШ_Продавец = ОбластьШ_Продавец_Нечет;
			ОбластьШ_БонусыПроцент = ОбластьШ_БонусыПроцент_Нечет;
			ОбластьШ_БонусыРубОсн = ОбластьШ_БонусыРубОсн_Нечет;
			ОбластьШ_БонусыРубУц = ОбластьШ_БонусыРубУц_Нечет;
			ОбластьШ_БонусыРуб = ОбластьШ_БонусыРуб_Нечет;
			ОбластьШ_Смены = ОбластьШ_Смены_Нечет;
		иначе
			ОбластьШ_Продавец = ОбластьШ_Продавец_Чет;
			ОбластьШ_БонусыПроцент = ОбластьШ_БонусыПроцент_Чет;
			ОбластьШ_БонусыРубОсн = ОбластьШ_БонусыРубОсн_Чет;
			ОбластьШ_БонусыРубУц = ОбластьШ_БонусыРубУц_Чет;
			ОбластьШ_БонусыРуб = ОбластьШ_БонусыРуб_Чет;
			ОбластьШ_Смены = ОбластьШ_Смены_Чет;
		конецесли;	
		
		Расшифровка = новый Структура;
		Расшифровка.Вставить("НоменклатураРодитель","");
		Расшифровка.Вставить("ДокументПродажиОтветственный",тзПродавцы[н].ДокументПродажиОтветственный);
		
		ТекстБонусыПроцент = ?(ПараметрБонусыПродавцам.ВыводитьПродажи,"Бонусы %",сокрлп(тзПродавцы[н].ДокументПродажиОтветственный)+", Бонусы %");
		ТекстБонусыРуб = ?(ПараметрБонусыПродавцам.ВыводитьПродажи или ПараметрБонусыПродавцам.ВыводитьПроценты,"Бонусы руб.",сокрлп(тзПродавцы[н].ДокументПродажиОтветственный)+", Бонусы руб.");
		ТекстСмены = ?(ПараметрБонусыПродавцам.ВыводитьПродажи или ПараметрБонусыПродавцам.ВыводитьПроценты или ПараметрБонусыПродавцам.ВыводитьБонусы,"Смены",сокрлп(тзПродавцы[н].ДокументПродажиОтветственный)+", Смены");
		ТекстБонусыРубОсн = "Бонус руб. Основной";
		ТекстБонусыРубУц = "Бонус руб. Уценка";
			
		если ПараметрБонусыПродавцам.ВыводитьПродажи тогда
			ОбластьШ_Продавец.Параметры.ДокументПродажиОтветственный = тзПродавцы[н].ДокументПродажиОтветственный;
			ОбластьШ_Продавец.Параметры.СтруктураРасшифровки = Расшифровка;
			ТабличныйДокумент.Присоединить(ОбластьШ_Продавец,0);
		конецесли;
		если ПараметрБонусыПродавцам.ВыводитьПроценты тогда
			ОбластьШ_БонусыПроцент.Параметры.ТекстБонусыПроцент = ТекстБонусыПроцент;
			ОбластьШ_БонусыПроцент.Параметры.СтруктураРасшифровки = Расшифровка;
			ТабличныйДокумент.Присоединить(ОбластьШ_БонусыПроцент,1);
		конецесли;	
		если ПараметрБонусыПродавцам.ВыводитьУценку тогда
			ОбластьШ_БонусыРубОсн.Параметры.ТекстБонусыРубОсн = ТекстБонусыРубОсн;
			ОбластьШ_БонусыРубОсн.Параметры.СтруктураРасшифровки = Расшифровка;
			ТабличныйДокумент.Присоединить(ОбластьШ_БонусыРубОсн,1);
			
			ОбластьШ_БонусыРубУц.Параметры.ТекстБонусыРубУц = ТекстБонусыРубУц;
			ОбластьШ_БонусыРубУц.Параметры.СтруктураРасшифровки = Расшифровка;
			ТабличныйДокумент.Присоединить(ОбластьШ_БонусыРубУц,1);
		конецесли;
		если ПараметрБонусыПродавцам.ВыводитьБонусы тогда
			ОбластьШ_БонусыРуб.Параметры.ТекстБонусыРуб = ТекстБонусыРуб;
			ОбластьШ_БонусыРуб.Параметры.СтруктураРасшифровки = Расшифровка;
			ТабличныйДокумент.Присоединить(ОбластьШ_БонусыРуб,1);
		конецесли;
		если ПараметрБонусыПродавцам.ВыводитьСмены тогда
			ОбластьШ_Смены.Параметры.ТекстСмены = ТекстСмены;
			ОбластьШ_Смены.Параметры.СтруктураРасшифровки = Расшифровка;
			ТабличныйДокумент.Присоединить(ОбластьШ_Смены,1);
		конецесли;
	конеццикла;	
	
	если ПараметрБонусыПродавцам.ВыводитьПродажи тогда
		ТабличныйДокумент.Присоединить(ОбластьШ_Всего,0);
	конецесли;		
	если ПараметрБонусыПродавцам.ВыводитьБонусы тогда
		ТабличныйДокумент.Присоединить(ОбластьШ_ВсегоБонусы,0);
	конецесли;	
	
	сч = 0;
	счК = тзГруппы.Количество()*тзПродавцы.Количество(); 
	для каждого стрГруппы из тзГруппы цикл
		
		Расшифровка = новый Структура;
		Расшифровка.Вставить("НоменклатураРодитель",стрГруппы.НоменклатураРодитель);
		Расшифровка.Вставить("ДокументПродажиОтветственный","");
		
		ОбластьСтрока_Осн.Параметры.НоменклатураРодитель = стрГруппы.НоменклатураРодитель;
		ОбластьСтрока_Осн.Параметры.СтруктураРасшифровки = Расшифровка;
		ТабличныйДокумент.Вывести(ОбластьСтрока_Осн,0);
		
		для н = 0 по тзПродавцы.Количество()-1 цикл
			сч = сч + 1;
			
			если окр(н / 2,0,1) <> н / 2 тогда
				ОбластьСтрока_Продавец = ОбластьСтрока_Продавец_Нечет;
				ОбластьСтрока_БонусыПроцент = ОбластьСтрока_БонусыПроцент_Нечет;
				ОбластьСтрока_БонусыРубОсн = ОбластьСтрока_БонусыРубОсн_Нечет;
				ОбластьСтрока_БонусыРубУц = ОбластьСтрока_БонусыРубУц_Нечет;
				ОбластьСтрока_БонусыРуб = ОбластьСтрока_БонусыРуб_Нечет;
				ОбластьСтрока_Смены = ОбластьСтрока_Смены_Нечет;
			иначе
				ОбластьСтрока_Продавец = ОбластьСтрока_Продавец_Чет;
				ОбластьСтрока_БонусыПроцент = ОбластьСтрока_БонусыПроцент_Чет;
				ОбластьСтрока_БонусыРубОсн = ОбластьСтрока_БонусыРубОсн_Чет;
				ОбластьСтрока_БонусыРубУц = ОбластьСтрока_БонусыРубУц_Чет;
				ОбластьСтрока_БонусыРуб = ОбластьСтрока_БонусыРуб_Чет;
				ОбластьСтрока_Смены = ОбластьСтрока_Смены_Чет;
			конецесли;
			
			стрПродавцы = тзПродавцы[н];
			...
			Отбор.Очистить();
			Отбор.Вставить("НоменклатураРодитель",стрГруппы.НоменклатураРодитель); 
			Отбор.Вставить("ДокументПродажиОтветственный",стрПродавцы.ДокументПродажиОтветственный);
			
			Расшифровка = новый Структура;
			Расшифровка.Вставить("НоменклатураРодитель",стрГруппы.НоменклатураРодитель);
			Расшифровка.Вставить("ДокументПродажиОтветственный",стрПродавцы.ДокументПродажиОтветственный);
			
			строки = тзГруппыПродавцы.НайтиСтроки(Отбор);
			если строки.Количество()>0 тогда
				СтоимостьОборот = строки[0].СтоимостьОборот;
				БонусПроцент = строки[0].БонусПроцент;
				БонусРуб = строки[0].БонусРуб;
				СменДней_0 = строки[0].СменДней;
				если ПараметрБонусыПродавцам.ВыводитьУценку тогда
					БонусРубОсн = строки[0].БонусРубОсн;
					БонусРубУц = строки[0].БонусРубУц;
				конецесли;	
			конецесли;
			
			если ПараметрБонусыПродавцам.ВыводитьПродажи тогда
				ОбластьСтрока_Продавец.Параметры.СтоимостьОборот = СтоимостьОборот;
				ОбластьСтрока_Продавец.Параметры.СтруктураРасшифровки = Расшифровка;
				ТабличныйДокумент.Присоединить(ОбластьСтрока_Продавец,0);
			конецесли;
			если ПараметрБонусыПродавцам.ВыводитьПроценты тогда
				ОбластьСтрока_БонусыПроцент.Параметры.БонусПроцент = БонусПроцент;
				ОбластьСтрока_БонусыПроцент.Параметры.СтруктураРасшифровки = Расшифровка;
				ТабличныйДокумент.Присоединить(ОбластьСтрока_БонусыПроцент,1);
			конецесли;	
			если ПараметрБонусыПродавцам.ВыводитьУценку тогда
				ОбластьСтрока_БонусыРубОсн.Параметры.БонусРубОсн = БонусРубОсн;
				ОбластьСтрока_БонусыРубОсн.Параметры.СтруктураРасшифровки = Расшифровка;
				ТабличныйДокумент.Присоединить(ОбластьСтрока_БонусыРубОсн,1);
				
				ОбластьСтрока_БонусыРубУц.Параметры.БонусРубУц = БонусРубУц;
				ОбластьСтрока_БонусыРубУц.Параметры.СтруктураРасшифровки = Расшифровка;
				ТабличныйДокумент.Присоединить(ОбластьСтрока_БонусыРубУц,1);
			конецесли;		
			если ПараметрБонусыПродавцам.ВыводитьБонусы тогда
				ОбластьСтрока_БонусыРуб.Параметры.БонусРуб = БонусРуб;
				ОбластьСтрока_БонусыРуб.Параметры.СтруктураРасшифровки = Расшифровка;
				ТабличныйДокумент.Присоединить(ОбластьСтрока_БонусыРуб,1);
			конецесли;
			если ПараметрБонусыПродавцам.ВыводитьСмены тогда
				ОбластьСтрока_Смены.Параметры.СменДней = СменДней_0;
				ОбластьСтрока_Смены.Параметры.СтруктураРасшифровки = Расшифровка;
				ТабличныйДокумент.Присоединить(ОбластьСтрока_Смены,1);
			конецесли;
			
			состояние("Вывод данных по продавцам "+сч+" из "+счК);
		конеццикла;	
		...
		если ПараметрБонусыПродавцам.ВыводитьПродажи тогда
			ОбластьСтрока_Всего.Параметры.СтоимостьОборот = стрГруппы.СтоимостьОборот;
			ОбластьСтрока_Всего.Параметры.СтруктураРасшифровки = Расшифровка;
			ТабличныйДокумент.Присоединить(ОбластьСтрока_Всего,0);
		конецесли;	
		если ПараметрБонусыПродавцам.ВыводитьБонусы тогда
			ОбластьСтрока_ВсегоБонусы.Параметры.БонусРуб = стрГруппы.БонусРуб;
			ОбластьСтрока_ВсегоБонусы.Параметры.СтруктураРасшифровки = Расшифровка;
			ТабличныйДокумент.Присоединить(ОбластьСтрока_ВсегоБонусы,0);
		конецесли;
	конеццикла;
	
	ТабличныйДокумент.Вывести(ОбластьИтог_Осн,0);
	для н = 0 по тзПродавцы.Количество()-1 цикл
		если окр(н / 2,0,1) <> н / 2 тогда
			ОбластьИтог_Продавец = ОбластьИтог_Продавец_Нечет;
			ОбластьИтог_БонусыПроцент = ОбластьИтог_БонусыПроцент_Нечет;
			ОбластьИтог_БонусыРубОсн = ОбластьИтог_БонусыРубОсн_Нечет;
			ОбластьИтог_БонусыРубУц = ОбластьИтог_БонусыРубУц_Нечет;
			ОбластьИтог_БонусыРуб = ОбластьИтог_БонусыРуб_Нечет;
			ОбластьИтог_Смены = ОбластьИтог_Смены_Нечет;
		иначе
			ОбластьИтог_Продавец = ОбластьИтог_Продавец_Чет;
			ОбластьИтог_БонусыПроцент = ОбластьИтог_БонусыПроцент_Чет;
			ОбластьИтог_БонусыРубОсн = ОбластьИтог_БонусыРубОсн_Чет;
			ОбластьИтог_БонусыРубУц = ОбластьИтог_БонусыРубУц_Чет;
			ОбластьИтог_БонусыРуб = ОбластьИтог_БонусыРуб_Чет;
			ОбластьИтог_Смены = ОбластьИтог_Смены_Чет;
		конецесли;	
		...
		СтрокиСмены = тзСмены.НайтиСтроки(новый Структура("ДокументПродажиОтветственный",тзПродавцы[н].ДокументПродажиОтветственный));
		СменДней = СтрокиСмены.Количество();
		
		если ПараметрБонусыПродавцам.ВыводитьПродажи тогда
			ОбластьИтог_Продавец.Параметры.СтоимостьОборот = тзПродавцы[н].СтоимостьОборот;
			ОбластьИтог_Продавец.Параметры.СтруктураРасшифровки = Расшифровка; 
			ТабличныйДокумент.Присоединить(ОбластьИтог_Продавец,0);
		конецесли;	
		если ПараметрБонусыПродавцам.ВыводитьПроценты тогда
			ОбластьИтог_БонусыПроцент.Параметры.СтруктураРасшифровки = Расшифровка; 
			ТабличныйДокумент.Присоединить(ОбластьИтог_БонусыПроцент,1);
		конецесли;
		если ПараметрБонусыПродавцам.ВыводитьУценку тогда
			ОбластьИтог_БонусыРубОсн.Параметры.БонусРубОсн = тзПродавцы[н].БонусРубОсн;
			ОбластьИтог_БонусыРубОсн.Параметры.СтруктураРасшифровки = Расшифровка; 
			ТабличныйДокумент.Присоединить(ОбластьИтог_БонусыРубОсн,1);
			
			ОбластьИтог_БонусыРубУц.Параметры.БонусРубУц = тзПродавцы[н].БонусРубУц;
			ОбластьИтог_БонусыРубУц.Параметры.СтруктураРасшифровки = Расшифровка; 
			ТабличныйДокумент.Присоединить(ОбластьИтог_БонусыРубУц,1);
		конецесли;
		если ПараметрБонусыПродавцам.ВыводитьБонусы тогда
			ОбластьИтог_БонусыРуб.Параметры.БонусРуб = тзПродавцы[н].БонусРуб;
			ОбластьИтог_БонусыРуб.Параметры.СтруктураРасшифровки = Расшифровка; 
			ТабличныйДокумент.Присоединить(ОбластьИтог_БонусыРуб,1);
		конецесли;
		если ПараметрБонусыПродавцам.ВыводитьСмены тогда
			ОбластьИтог_Смены.Параметры.СменДней = СменДней;
			ОбластьИтог_Смены.Параметры.СтруктураРасшифровки = Расшифровка; 
			ТабличныйДокумент.Присоединить(ОбластьИтог_Смены,1);
		конецесли;
	конеццикла;	
	...
	если ПараметрБонусыПродавцам.ВыводитьПродажи тогда
		ОбластьИтог_Всего.Параметры.СтоимостьОборот = тзПродавцы.Итог("СтоимостьОборот");
		ОбластьИтог_Всего.Параметры.СтруктураРасшифровки = Расшифровка;
		ТабличныйДокумент.Присоединить(ОбластьИтог_Всего,0);
	конецесли;	
	если ПараметрБонусыПродавцам.ВыводитьБонусы тогда
		ОбластьИтог_ВсегоБонусы.Параметры.БонусРуб = тзПродавцы.Итог("БонусРуб");
		ОбластьИтог_ВсегоБонусы.Параметры.СтруктураРасшифровки = Расшифровка;
		ТабличныйДокумент.Присоединить(ОбластьИтог_ВсегоБонусы,0);
	конецесли;	
...
Функция ТекстЗапросаБонусыПродавцов(ПараметрБонусыПродавцам)
	Если ПараметрБонусыПродавцам.ИнтернетМагазин тогда
		ЗапросТекст = "ВЫБРАТЬ
		...
		|       ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.БонусыСроки.СрезПоследних(&ДатаКон, ) КАК СрокиБонусов
		|	      ПО Бренды4Отчёта.Ссылка = СрокиБонусов.Папка
		|	ГДЕ
		|		НЕ Бренды4Отчёта.ПометкаУдаления
		//|       И НЕ СрокиБонусов.ПроцентПлан = 0
		...
		|	ВЫБОР КОГДА ЗаказТовары.Менеджер ЕСТЬ NULL ТОГДА ИсточникДанных.ДокументПродажи.Сделка.Манагер ИНАЧЕ ЗаказТовары.Менеджер КОНЕЦ КАК ДокументПродажиОтветственный,
		|	ИсточникДанных.СтоимостьОборот КАК СтоимостьОборот,
		|	НАЧАЛОПЕРИОДА(ИсточникДанных.ДокументПродажи.Дата, ДЕНЬ) КАК ДокументПродажиДата,
		|	тзБонусПроцент.БонусПроцент,
		|	ЕСТЬNULL(тзБонусПроцент.Папка, ""Итого остальных продаж"") КАК Папка
		|ПОМЕСТИТЬ тзДанные0
		|ИЗ
		...
		|       ПО ЗаказТовары.Ссылка = ИсточникДанных.ДокументПродажи.Сделка
		|       И ЗаказТовары.Номенклатура = ИсточникДанных.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ тзБонусПроцент КАК тзБонусПроцент
		|		ПО ВЫБОР КОГДА тзБонусПроцент.Папка.ЭтоГруппа Тогда ВЫБОР
		|					КОГДА ИсточникДанных.Номенклатура.Родитель = тзБонусПроцент.Папка
		|						ТОГДА ИсточникДанных.Номенклатура.Родитель = тзБонусПроцент.Папка
		|					ИНАЧЕ ИсточникДанных.Номенклатура.Родитель.Родитель = тзБонусПроцент.Папка
		|				КОНЕЦ ИНАЧЕ ИсточникДанных.Номенклатура = тзБонусПроцент.Папка КОНЕЦ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	тзДанные0.Номенклатура,
		|	тзДанные0.НоменклатураРодитель,
		|	тзДанные0.ДокументПродажи,
		|	тзДанные0.ДокументПродажиОтветственный,
		|	тзДанные0.СтоимостьОборот,
		|	тзДанные0.ДокументПродажиДата,
		|	ВЫБОР
		|		КОГДА тзДанные0.Папка = ""Итого остальных продаж""
		|			ТОГДА 0.5
		|		ИНАЧЕ тзДанные0.БонусПроцент
		|	КОНЕЦ КАК БонусПроцент,
		|	тзДанные0.Папка
		|ПОМЕСТИТЬ тзДанные
		|ИЗ
		|	тзДанные0 КАК тзДанные0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	тзДанные.Папка КАК НоменклатураРодитель,
		|	тзДанные.ДокументПродажи,
		|	тзДанные.ДокументПродажиОтветственный,
		|	тзДанные.ДокументПродажиДата,
		|	СУММА(тзДанные.СтоимостьОборот) КАК СтоимостьОборот,
		|	тзДанные.БонусПроцент,
		|	СУММА(ВЫРАЗИТЬ(тзДанные.СтоимостьОборот * (тзДанные.БонусПроцент / 100) КАК ЧИСЛО(15, 2))) КАК БонусРуб,
		|	СУММА(0) КАК СменДней
		|ИЗ
		|	тзДанные КАК тзДанные
		|ГДЕ тзДанные.ДокументПродажиОтветственный НЕ В (&НеМанагер)
		|
		|СГРУППИРОВАТЬ ПО
		|	тзДанные.Папка,
		|	тзДанные.ДокументПродажи,
		...
		|	тзДанные.ДокументПродажиДата,
		|	тзДанные.БонусПроцент";
	ИначеЕсли ПараметрБонусыПродавцам.ВыводитьУценку тогда
		ЗапросТекст = "ВЫБРАТЬ
	               |	ВложенныйЗапрос.Папка,
	               |	ВложенныйЗапрос.БонусПроцент
	               |ПОМЕСТИТЬ тзБонусПроцент
	              ...
	              // |		И НЕ СрокиБонусов.ПроцентПлан = 0
	               ...
	               |	ИсточникДанных.Номенклатура.Родитель КАК НоменклатураРодитель,
	               |	ИсточникДанных.ДокументПродажи КАК ДокументПродажи,
	               |	ИсточникДанных.ДокументПродажи.Ответственный КАК ДокументПродажиОтветственный,
	               |	ИсточникДанных.СтоимостьОборот КАК СтоимостьОборот,
	               |	НАЧАЛОПЕРИОДА(ИсточникДанных.ДокументПродажи.Дата, ДЕНЬ) КАК ДокументПродажиДата,
	               |	тзБонусПроцент.БонусПроцент,
	               |	ВЫБОР
	               |		КОГДА тзБонусПроцент.Папка.ЭтоГруппа
	               |			ТОГДА тзБонусПроцент.Папка
	               |		ИНАЧЕ тзБонусПроцент.Папка.Родитель
	               |	КОНЕЦ КАК Папка,
	               ...
	               |	тзДанные.НоменклатураРодитель,
	               |	тзДанные.ДокументПродажи,
	               |	тзДанные.ДокументПродажиОтветственный,
	               |	тзДанные.СтоимостьОборот,
	               |	тзДанные.ДокументПродажиДата,
	               |	тзДанные.БонусПроцент,
	               |	тзДанные.Папка,
	               |	ТоварыНаСкладахОбороты.Качество,
	               |	тзДанные.КоличествоОборот
	               |ПОМЕСТИТЬ тзДанныеКачество
	               |ИЗ
	               |	тзДанные КАК тзДанные
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.Обороты(&ДатаНач, &ДатаКон, Регистратор, ) КАК ТоварыНаСкладахОбороты
	               |		ПО тзДанные.ДокументПродажи = ТоварыНаСкладахОбороты.Регистратор
	               |			И тзДанные.Номенклатура = ТоварыНаСкладахОбороты.Номенклатура
	               |ГДЕ
	               |	НЕ тзДанные.Папка ЕСТЬ NULL 
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	тзДанныеКачество.Номенклатура,
	               |	тзДанныеКачество.НоменклатураРодитель,
	               |	тзДанныеКачество.ДокументПродажи,
	               |	тзДанныеКачество.ДокументПродажиОтветственный,
	               |	тзДанныеКачество.СтоимостьОборот,
	               |	тзДанныеКачество.ДокументПродажиДата,
	               |	тзДанныеКачество.БонусПроцент,
	               |	тзДанныеКачество.Папка,
	               |	тзДанныеКачество.Качество,
	               |	ЕСТЬNULL(ПроцентыЗаУценкуСрезПоследних.Скидка, 0) КАК Скидка,
	               |	ЕСТЬNULL(ПроцентыЗаУценкуСрезПоследних.БонусПродавцу, 0) КАК БонусПродавцу,
	               |	ЕСТЬNULL(ПроцентыЗаУценкуСрезПоследних.ВычетСПродавца, 0) КАК ВычетСПродавца,
	               |	ЕСТЬNULL(ПроцентыЗаУценкуСрезПоследних.ВычетСоСклада, 0) КАК ВычетСоСклада,
	               |	тзДанныеКачество.КоличествоОборот
	               |ПОМЕСТИТЬ тзДанныеКачествоПроценты
	               |ИЗ
	               |	тзДанныеКачество КАК тзДанныеКачество
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПроцентыЗаУценку.СрезПоследних(&ДатаНач, ) КАК ПроцентыЗаУценкуСрезПоследних
	               |		ПО тзДанныеКачество.Качество = ПроцентыЗаУценкуСрезПоследних.Причина
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	тзДанныеКачествоПроценты.Номенклатура,
	               |	тзДанныеКачествоПроценты.НоменклатураРодитель,
	               |	тзДанныеКачествоПроценты.ДокументПродажи,
	               |	тзДанныеКачествоПроценты.ДокументПродажиОтветственный,
	               |	тзДанныеКачествоПроценты.СтоимостьОборот,
	               |	тзДанныеКачествоПроценты.ДокументПродажиДата,
	               |	тзДанныеКачествоПроценты.БонусПроцент,
	               |	тзДанныеКачествоПроценты.Папка,
	               |	тзДанныеКачествоПроценты.Качество,
	               |	тзДанныеКачествоПроценты.Скидка,
	               |	тзДанныеКачествоПроценты.БонусПродавцу,
	               |	тзДанныеКачествоПроценты.ВычетСПродавца,
	               |	тзДанныеКачествоПроценты.ВычетСоСклада,
	               |	ВЫБОР
	               |		КОГДА тзДанныеКачествоПроценты.Номенклатура.ЕдиницаХраненияОстатков.ЕдиницаПоКлассификатору <> ЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения.ЕдиницаПоКлассификатору
	               |				И ЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения.Коэффициент > 0
	               |			ТОГДА ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0) / ЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения.Коэффициент
	               |		ИНАЧЕ ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0)
	               |	КОНЕЦ КАК Цена5,
	               |	тзДанныеКачествоПроценты.КоличествоОборот
	               |ПОМЕСТИТЬ тзДанныеКачествоИтог
	               |ИЗ
	               |	тзДанныеКачествоПроценты КАК тзДанныеКачествоПроценты
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ДатаКон, ТипЦен = &ТипЦен5) КАК ЦеныНоменклатурыСрезПоследних
	               |		ПО тзДанныеКачествоПроценты.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	тзДанныеКачествоИтог.Номенклатура,
	               |	тзДанныеКачествоИтог.НоменклатураРодитель,
	               |	тзДанныеКачествоИтог.ДокументПродажи,
	               |	тзДанныеКачествоИтог.ДокументПродажиОтветственный,
	               |	тзДанныеКачествоИтог.СтоимостьОборот,
	               |	тзДанныеКачествоИтог.ДокументПродажиДата,
	               |	тзДанныеКачествоИтог.Папка,
	               |	ЕСТЬNULL(тзДанныеКачествоИтог.БонусПроцент, 0) КАК БонусПроцент,
	               |	ЕСТЬNULL(ВЫРАЗИТЬ(тзДанныеКачествоИтог.СтоимостьОборот * (тзДанныеКачествоИтог.БонусПроцент / 100) КАК ЧИСЛО(15, 2)), 0) КАК БонусРубОсн,
	               |	тзДанныеКачествоИтог.Качество,
	               |	тзДанныеКачествоИтог.Скидка,
	               |	тзДанныеКачествоИтог.БонусПродавцу,
	               |	ВЫРАЗИТЬ(тзДанныеКачествоИтог.СтоимостьОборот * (тзДанныеКачествоИтог.БонусПродавцу / 100) КАК ЧИСЛО(15, 2)) КАК БонусПродавцуРуб,
	               |	тзДанныеКачествоИтог.ВычетСПродавца,
	               |	тзДанныеКачествоИтог.КоличествоОборот,
	               |	тзДанныеКачествоИтог.Цена5,
	               |	ВЫРАЗИТЬ(тзДанныеКачествоИтог.КоличествоОборот * тзДанныеКачествоИтог.Цена5 * (тзДанныеКачествоИтог.ВычетСПродавца / 100) КАК ЧИСЛО(15, 2)) КАК ВычетСПродавцаРуб,
	               |	тзДанныеКачествоИтог.ВычетСоСклада
	               |ПОМЕСТИТЬ тзДанныеКачествоИтогПередВыводом
	               |ИЗ
	               |	тзДанныеКачествоИтог КАК тзДанныеКачествоИтог
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	тзДанныеКачествоИтогПередВыводом.Номенклатура,
	               |	тзДанныеКачествоИтогПередВыводом.Папка КАК НоменклатураРодитель,
	               |	тзДанныеКачествоИтогПередВыводом.ДокументПродажи,
	               |	тзДанныеКачествоИтогПередВыводом.ДокументПродажиОтветственный,
	               |	тзДанныеКачествоИтогПередВыводом.СтоимостьОборот КАК СтоимостьОборот,
	               |	тзДанныеКачествоИтогПередВыводом.ДокументПродажиДата,
	               |	тзДанныеКачествоИтогПередВыводом.БонусПроцент,
	               |	тзДанныеКачествоИтогПередВыводом.БонусРубОсн КАК БонусРубОсн,
	               |	тзДанныеКачествоИтогПередВыводом.Качество,
	               |	тзДанныеКачествоИтогПередВыводом.БонусПродавцу КАК БонусПродавцу,
	               |	тзДанныеКачествоИтогПередВыводом.БонусПродавцуРуб КАК БонусПродавцуРуб,
	               |	тзДанныеКачествоИтогПередВыводом.ВычетСПродавца КАК ВычетСПродавца,
	               |	тзДанныеКачествоИтогПередВыводом.КоличествоОборот КАК КоличествоОборот,
	               |	тзДанныеКачествоИтогПередВыводом.Цена5 КАК Цена5lb,
	               |	тзДанныеКачествоИтогПередВыводом.ВычетСПродавцаРуб КАК ВычетСПродавцаРуб,
	               |	тзДанныеКачествоИтогПередВыводом.БонусПродавцуРуб - тзДанныеКачествоИтогПередВыводом.ВычетСПродавцаРуб КАК БонусРубУц,
	               |	тзДанныеКачествоИтогПередВыводом.БонусРубОсн + тзДанныеКачествоИтогПередВыводом.БонусПродавцуРуб - тзДанныеКачествоИтогПередВыводом.ВычетСПродавцаРуб КАК БонусРуб,
	               |	0 КАК СменДней
	               |ИЗ
	               |	тзДанныеКачествоИтогПередВыводом КАК тзДанныеКачествоИтогПередВыводом
	               |
	               |УПОРЯДОЧИТЬ ПО
				   |  тзДанныеКачествоИтогПередВыводом.Папка,
				   |  тзДанныеКачествоИтогПередВыводом.ДокументПродажиОтветственный,
	               |  тзДанныеКачествоИтогПередВыводом.ДокументПродажи";
		...
		//              |	НАЧАЛОПЕРИОДА(ИсточникДанных.ДокументПродажи.Дата, ДЕНЬ) КАК ДокументПродажиДата,
		//              |	тзБонусПроцент.БонусПроцент,
		//             // |	ЕСТЬNULL(тзБонусПроцент.Папка, ""НЕМОТИВИРУЕТСЯ"") КАК Папка,
		//			 |ВЫБОР
		//			 |КОГДА тзБонусПроцент.Папка.ЭтоГруппа
		//			 |		ТОГДА тзБонусПроцент.Папка
		//			 | ИНАЧЕ тзБонусПроцент.Папка.Родитель
		...
		//              |	РегистрНакопления.Продажи.Обороты(&ДатаНач, &ДатаКон, Регистратор {(&Периодичность)}, {(Номенклатура).* КАК Номенклатура, (ХарактеристикаНоменклатуры).* КАК ХарактеристикаНоменклатуры, (ЗаказПокупателя).* КАК ЗаказПокупателя, (ДоговорКонтрагента).* КАК ДоговорКонтрагента, (ДокументПродажи).* КАК ДокументПродажи, (Подразделение).* КАК Подразделение, (Проект).* КАК Проект, (Организация).* КАК Организация, (Контрагент).* КАК Контрагент, (Номенклатура.БазоваяЕдиницаИзмерения).* КАК НоменклатураБазоваяЕдиницаИзмерения}) КАК ИсточникДанных
		//              |		ЛЕВОЕ СОЕДИНЕНИЕ тзБонусПроцент КАК тзБонусПроцент
		//              |		ПО ВЫБОР КОГДА тзБонусПроцент.Папка.ЭтоГруппа Тогда ИсточникДанных.Номенклатура.Родитель = тзБонусПроцент.Папка ИНАЧЕ ИсточникДанных.Номенклатура = тзБонусПроцент.Папка КОНЕЦ
		//              |;
		//              |
		//              |////////////////////////////////////////////////////////////////////////////////
		//              |ВЫБРАТЬ
		...
		//              |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.Обороты(&ДатаНач, &ДатаКон, Регистратор, ) КАК ТоварыНаСкладахОбороты
		//              |		ПО тзДанные.ДокументПродажи = ТоварыНаСкладахОбороты.Регистратор
		//			  |			И ВЫБОР КОГДА тзДанные.Папка.ЭтоГруппа Тогда ТзДанные.Папка = ТоварыНаСкладахОбороты.Номенклатура.Родитель ИНАЧЕ ТзДанные.Папка = ТоварыНаСкладахОбороты.Номенклатура  КОНЕЦ
		//              |;
		//              |
		//              |////////////////////////////////////////////////////////////////////////////////
		//              |ВЫБРАТЬ
		...
		//			  ////|	тзДанныеКачествоИтогПередВыводом.Номенклатура,
		//			  //|	тзДанныеКачествоИтогПередВыводом.Папка КАК НоменклатураРодитель,
		//			  ////|	тзДанныеКачествоИтогПередВыводом.НоменклатураРодитель,
		//			  //|	тзДанныеКачествоИтогПередВыводом.ДокументПродажи,
		//			  //|	тзДанныеКачествоИтогПередВыводом.ДокументПродажиОтветственный,
		//			  //|	СУММА(тзДанныеКачествоИтогПередВыводом.СтоимостьОборот) КАК СтоимостьОборот,
		//			  //|	тзДанныеКачествоИтогПередВыводом.ДокументПродажиДата,
		//			  //|	тзДанныеКачествоИтогПередВыводом.БонусПроцент,
		//			  //|	СУММА(тзДанныеКачествоИтогПередВыводом.БонусРубОсн) КАК БонусРубОсн,
		//			  //|	тзДанныеКачествоИтогПередВыводом.Качество,
		//			  //|	СУММА(тзДанныеКачествоИтогПередВыводом.БонусПродавцу) КАК БонусПродавцу,
		//			  //|	СУММА(тзДанныеКачествоИтогПередВыводом.БонусПродавцуРуб) КАК БонусПродавцуРуб,
		//			  //|	СУММА(тзДанныеКачествоИтогПередВыводом.ВычетСПродавца) КАК ВычетСПродавца,
		//			  //|	СУММА(тзДанныеКачествоИтогПередВыводом.КоличествоОборот) КАК КоличествоОборот,
		//			  //|	Максимум(тзДанныеКачествоИтогПередВыводом.Цена5) КАК Цена5lb,
		//			  //|	СУММА(тзДанныеКачествоИтогПередВыводом.ВычетСПродавцаРуб) КАК ВычетСПродавцаРуб,
		//			  //|	СУММА(тзДанныеКачествоИтогПередВыводом.БонусПродавцуРуб - тзДанныеКачествоИтогПередВыводом.ВычетСПродавцаРуб) КАК БонусРубУц,
		//			  //|	СУММА(тзДанныеКачествоИтогПередВыводом.БонусРубОсн + тзДанныеКачествоИтогПередВыводом.БонусПродавцуРуб - тзДанныеКачествоИтогПередВыводом.ВычетСПродавцаРуб) КАК БонусРуб,
		//			  //|	0 КАК СменДней
		//			  //|ИЗ
		//			  //|	тзДанныеКачествоИтогПередВыводом КАК тзДанныеКачествоИтогПередВыводом
		//			  //|СГРУППИРОВАТЬ ПО 
	...
	Иначе	
		...
		| ИНАЧЕ тзБонусПроцент.Папка.Родитель
		|    КОНЕЦ КАК Папка
		//| тзБонусПроцент.Папка КАК Папка
		|ПОМЕСТИТЬ тзДанные
		|ИЗ
		|	РегистрНакопления.Продажи.Обороты(&ДатаНач, &ДатаКон, Регистратор {(&Периодичность)}, {(Номенклатура).* КАК Номенклатура, (ХарактеристикаНоменклатуры).* КАК ХарактеристикаНоменклатуры, (ЗаказПокупателя).* КАК ЗаказПокупателя, (ДоговорКонтрагента).* КАК ДоговорКонтрагента, (ДокументПродажи).* КАК ДокументПродажи, (Подразделение).* КАК Подразделение, (Проект).* КАК Проект, (Организация).* КАК Организация, (Контрагент).* КАК Контрагент, (Номенклатура.БазоваяЕдиницаИзмерения).* КАК НоменклатураБазоваяЕдиницаИзмерения}) КАК ИсточникДанных
		|		ЛЕВОЕ СОЕДИНЕНИЕ тзБонусПроцент КАК тзБонусПроцент
		...
		|ВЫБРАТЬ
		|	тзДанные.Папка КАК НоменклатураРодитель,
		|	тзДанные.ДокументПродажи КАК ДокументПродажи,
		|	тзДанные.ДокументПродажиОтветственный КАК ДокументПродажиОтветственный,
		|	тзДанные.ДокументПродажиДата,
		|	СУММА(тзДанные.СтоимостьОборот) КАК СтоимостьОборот,
		|	тзДанные.БонусПроцент,
		|	СУММА(ВЫРАЗИТЬ(тзДанные.СтоимостьОборот * (тзДанные.БонусПроцент / 100) КАК ЧИСЛО(15, 2))) КАК БонусРуб,
...
//pryan@inbox.ru 15/02/2014
//Процедура СформироватьОтчет(ТабличныйДокумент, Очищать = Истина, ФиксироватьЗаголовок = Истина, Отчет = Неопределено), ПараметрБонусыПродавцам = неопределено, БонусыМакет = неопределено) Экспорт
Процедура СформироватьОтчет(ТабличныйДокумент, Очищать = Истина, ФиксироватьЗаголовок = Истина, Отчет = Неопределено, ПараметрБонусыПродавцам = неопределено, БонусыМакет = неопределено) Экспорт
	...
	Если ВыводВозможен() Тогда
		...
		если ВремяКон = Дата('00010101000000') тогда
		...
		иначеесли ДатаКон <> Дата('00010101000000') тогда
			пДатаКон = Дата(Год(ДатаКон),Месяц(ДатаКон),День(ДатаКон),Час(ВремяКон),Минута(ВремяКон),Секунда(ВремяКон));
			ПостроительОтчета.Параметры.Вставить("ДатаКон", Новый Граница(пДатаКон, ВидГраницы.Включая));
			ПостроительОтчета.Параметры.Вставить("ДатаКонца", Новый Граница(пДатаКон, ВидГраницы.Включая));
		иначеесли ДатаКон = Дата('00010101000000') тогда
			ПостроительОтчета.Параметры.Вставить("ДатаКон", ДатаКон);
			ПостроительОтчета.Параметры.Вставить("ДатаКонца", Дата("39991231"));
		конецесли;
		...
		//Добавил НеЯ
		Если (Отчет = "Продажи") И (РольДоступна(Метаданные.Роли.Пользователь)) И (НЕ РольДоступна(Метаданные.Роли.Collector)) Тогда
			
			Результат = "";
				ВвестиСтроку(Результат, "Наберите Ваш PIN");
				Если Не ПустаяСтрока(Результат) Тогда
					ТекМанагер = Справочники.Пользователи.НайтиПоКоду(Результат);
					Если (ТекМанагер<>Справочники.Пользователи.ПустаяСсылка()) И (ТекМанагер.ЭтоГруппа=Ложь) Тогда
						...
						|   ЗначениеНастроек.Пользователь = &Юзер
						|И  ЗначениеНастроек.Настройка = &Скл";
						Запрос2.УстановитьПараметр("Юзер", ТекМанагер);
						Запрос2.УстановитьПараметр("Скл", ПланыВидовХарактеристик.НастройкиПользователей.ОсновнойСклад);
						
						Рез = Запрос2.Выполнить();
						Если Рез.Пустой() Тогда
							Сообщить ("Указанный PIN код "+Результат+" не прикреплён к магазину!");
							возврат;
						КонецЕсли;
						
						Выборка = Рез.Выбрать();
						Пока Выборка.Следующий() Цикл
							СкладПИН = Выборка.Значение;
						КонецЦикла;

						Если СкладТекЮзера <> СкладПИН Тогда
							Предупреждение ("Вы не можете форировать отчёт по указанному PIN коду");
							возврат;
						КонецЕсли;
						
						Попытка
							ПостроительОтчета.Отбор.РегистраторОтветственный.Значение = ТекМанагер;
						Исключение
							ПостроительОтчета.Отбор.Добавить("Регистратор.Ответственный");
							ПостроительОтчета.Отбор.РегистраторОтветственный.Использование=Истина;
							ПостроительОтчета.Отбор.РегистраторОтветственный.ВидСравнения = ВидСравнения.Равно;
							ПостроительОтчета.Отбор.РегистраторОтветственный.Значение = ТекМанагер;
						КонецПопытки;
						
						
						СписокБрендов = Новый СписокЗначений;
						ВыбБренды = Справочники.Бренды4Отчёта.Выбрать();
						Пока ВыбБренды.Следующий() Цикл
							Если (ВыбБренды.Папка<>Справочники.Номенклатура.ПустаяСсылка()) И (ВыбБренды.ПометкаУдаления=Ложь) Тогда
							СписокБрендов.Добавить(ВыбБренды.Папка);
							КонецЕсли;
						КонецЦикла;
						
						Попытка
							ПостроительОтчета.Отбор.Номенклатура.Значение = СписокБрендов;
						Исключение
					...
					Иначе
						Предупреждение ("По указанному PIN коду продавец не найден!");  
						возврат;
					КонецЕсли;
				Иначе 
					Предупреждение ("Не был указан PIN код!");  
					возврат;
				КонецЕсли;
			
		КонецЕсли;
		
		//++ Кузнецов С.А. - Убрал из условия установки дат пользователя "Nataliya" (Задача в МегаПлане №3227) - 21.04.2016
		Если (ПользователиИнформационнойБазы.ТекущийПользователь().Имя = "Ivan") ИЛИ (ПользователиИнформационнойБазы.ТекущийПользователь().Имя = "Rail") Тогда
			ПостроительОтчета.Параметры.Вставить("ДатаНач", ?(ДатаНач <(ТекущаяДата()-1814400), ТекущаяДата()-1814400, Новый Граница(НачалоДня(ДатаНач), ВидГраницы.Включая)));
			ПостроительОтчета.Параметры.Вставить("ДатаКон", ?(ДатаКон = Дата('00010101000000'), ДатаКон, Новый Граница(КонецДня(ДатаКон), ВидГраницы.Включая)));
			ПостроительОтчета.Параметры.Вставить("ДатаНачала", ?(ДатаНач <(ТекущаяДата()-1814400), ТекущаяДата()-1814400, НачалоДня(ДатаНач)));
			ПостроительОтчета.Параметры.Вставить("ДатаКонца", ?(ДатаКон = Дата('00010101000000'), Дата("39991231"), КонецДня(ДатаКон)));
		КонецЕсли;		
		
		Если (ПользователиИнформационнойБазы.ТекущийПользователь().Имя = "Ivan") 
			ИЛИ (ПользователиИнформационнойБазы.ТекущийПользователь().Имя = "Rail")
			ИЛИ (ПользователиИнформационнойБазы.ТекущийПользователь().Имя = "Nataliya")
			ИЛИ (ПользователиИнформационнойБазы.ТекущийПользователь().Имя = "Tezieva Elena")

			Тогда
			Попытка
				Попытка
					ПостроительОтчета.Отбор.ЗаказПокупателяРегион.Значение = "Москва";
				Исключение
					ПостроительОтчета.Отбор.Добавить("ЗаказПокупателя.Регион");
					ПостроительОтчета.Отбор.ЗаказПокупателяРегион.Значение = "Москва";
				КонецПопытки;
				ПостроительОтчета.Отбор.ЗаказПокупателяРегион.Использование=Истина;
				ПостроительОтчета.Отбор.ЗаказПокупателяРегион.ВидСравнения = ВидСравнения.Равно;
			Исключение
			КонецПопытки;
		КонецЕсли;		
		//-- Кузнецов С.А. - 21.04.2016
		
		
//pryan@inbox.ru 15.02.2014		
		//ПостроительОтчета.Выполнить();
		//ПостроительОтчета.Вывести(ТабличныйДокумент);
		ВыводПостроителем = Истина;
		если ПараметрБонусыПродавцам <> неопределено тогда
			если ТипЗнч(ПараметрБонусыПродавцам) = Тип("Структура") тогда
				если ПараметрБонусыПродавцам.БонусыПродавцам тогда
					ВыводПостроителем = Ложь;
				конецесли;
			конецесли;
		конецесли;	
				
		если ВыводПостроителем тогда
			ПостроительОтчета.Выполнить();
			ПостроительОтчета.Вывести(ТабличныйДокумент);
		иначе
//сохранить для восстановления построителя
			ПостроительТекст = ПостроительОтчета.Текст;
			мПостроительИзмеренияСтроки = новый Массив;
			для н = 0 по ПостроительОтчета.ИзмеренияСтроки.Количество()-1 цикл
				мПостроительИзмеренияСтроки.Добавить(ПостроительОтчета.ИзмеренияСтроки[н]);
			конеццикла;	
			мПостроительИзмеренияКолонки = новый Массив;
			для н = 0 по ПостроительОтчета.ИзмеренияКолонки.Количество()-1 цикл
				мПостроительИзмеренияКолонки.Добавить(ПостроительОтчета.ИзмеренияКолонки[н]);
			конеццикла;	
			ПостроительМакет = ПостроительОтчета.Макет;
			ПостроительМакетОформления = ПостроительОтчета.МакетОформления;
			
			СписокНеМанагеров = Новый СписокЗначений;
			СписокНеМанагеров.Добавить(Справочники.Пользователи.НайтиПоКоду("11"));
			СписокНеМанагеров.Добавить(Справочники.Пользователи.НайтиПоКоду("45112"));
			СписокНеМанагеров.Добавить(Справочники.Пользователи.НайтиПоКоду("16"));
			СписокНеМанагеров.Добавить(Справочники.Пользователи.НайтиПоКоду("99"));
			СписокНеМанагеров.Добавить(Справочники.Пользователи.НайтиПоКоду("674"));
			СписокНеМанагеров.Добавить(Справочники.Пользователи.НайтиПоКоду("1"));
			
			ТипЦен5lb = Справочники.ТипыЦенНоменклатуры.НайтиПоКоду("000000001"); //5lb
			
			ПостроительОтчета.Текст = ТекстЗапросаБонусыПродавцов(ПараметрБонусыПродавцам);
			ПостроительОтчета.Параметры.Вставить("НеМанагер", СписокНеМанагеров);
			ПостроительОтчета.Параметры.Вставить("ТипЦен5", ТипЦен5lb);
			
			ПостроительОтчета.Выполнить();
			тзРезультат = ПостроительОтчета.Результат.Выгрузить();
			
			СформироватьОтчетБонусы(ТабличныйДокумент,БонусыМакет,ПараметрБонусыПродавцам,тзРезультат);
			
//восстановление построителя
			ПостроительОтчета.Текст = ПостроительТекст;
			для н = 0 по мПостроительИзмеренияСтроки.Количество()-1 цикл
				ПостроительОтчета.ИзмеренияСтроки.Добавить(мПостроительИзмеренияСтроки[н].ПутьКДанным,мПостроительИзмеренияСтроки[н].Имя,мПостроительИзмеренияСтроки[н].ТипИзмерения);
			конеццикла;	
			для н = 0 по мПостроительИзмеренияКолонки.Количество()-1 цикл
				ПостроительОтчета.ИзмеренияКолонки.Добавить(мПостроительИзмеренияКолонки[н].ПутьКДанным,мПостроительИзмеренияКолонки[н].Имя,мПостроительИзмеренияКолонки[н].ТипИзмерения);
			конеццикла;	
			ПостроительОтчета.Макет = ПостроительМакет;
			ПостроительОтчета.МакетОформления = ПостроительМакетОформления;
		конецесли;
//pryan@inbox.ru 15.02.2014		
...
Процедура ОбработкаРасшифровкиУниверсальногоОтчета(Расшифровка, Отчет, ДополнительныеПараметры = Неопределено, ПараметрБонусыПродавцам = неопределено) Экспорт
	
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
//pryan@inbox.ru 15/02/2014
		если ТипЗнч(ПараметрБонусыПродавцам) = Тип("Структура") тогда
			если ПараметрБонусыПродавцам.БонусыПродавцам тогда
				РасшифровкаБонусов(Расшифровка, ПараметрБонусыПродавцам);
				возврат;
			конецесли;
		конецесли;
//pryan@inbox.ru 15/02/2014
